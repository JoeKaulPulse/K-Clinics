...
export default async function ManageBookingPage({ searchParams }: { searchParams: Promise<{ t?: string }> }) {
  ...
  let booking: { treatmentTitle: string; treatmentSlug: string; startISO: string; status: string; pricePence: number; within24h: boolean; cancelled: boolean; rescheduleCount: number } | null = null;
  if (crmEnabled && t) {
    try {
