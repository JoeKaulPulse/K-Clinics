...
import { WaitlistRowActions, AddToWaitlistButton } from '@/components/admin/WaitlistActions';
...
export default async function WaitlistPage() {
  ...
  // BLD-1646: notify/remove/manual-add are actions, so they need bookings.manage
  // — the same bar the API routes enforce — not just the read-only bookings.view
  // that unlocks this page.
  const canManage = sessionCan(session, 'bookings.manage');
  ...
  const isoDay = (x: Date) => new Date(x).toISOString().slice(0, 10);
  // Same treatments the public waitlist join accepts (app/api/waitlist/route.ts) — no category filter.
  const treatmentOptions = treatments.map((t) => ({ slug: t.slug, title: t.title }));
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-[var(--color-stone)]">
            Clients waiting for a slot to free up. When a matching appointment is cancelled, the first waiting client is emailed automatically — or notify someone directly below once you've agreed a slot by phone.
          </p>
        </div>
        {canManage && <AddToWaitlistButton treatments={treatmentOptions} />}
      </div>
      ...
      <div className="mt-6 overflow-x-auto rounded-[var(--radius-lg)] border border-[var(--color-line)]">
        <table className="w-full min-w-[640px] text-left text-sm">
          <thead className="bg-[var(--color-bone)] text-xs uppercase tracking-wide text-[var(--color-stone)]">
            <tr>{['Client', 'Treatment', 'Window', 'Status', 'Waiting since', ...(canManage ? ['Actions'] : [])].map((h) => <th key={h} className="px-4 py-3 font-medium">{h}</th>)}</tr>
          </thead>
          <tbody className="divide-y divide-[var(--color-line)]">
            {rows.length === 0 && <tr><td colSpan={canManage ? 6 : 5} className="px-4 py-5 text-[var(--color-stone)]">No one is on the waitlist right now.</td></tr>}
            {rows.map((w) => (
              <tr key={w.id} className="bg-[var(--color-porcelain)] align-top transition-colors duration-150 hover:bg-[var(--color-bone)]">
                ...
                {canManage && (
                  <td className="px-4 py-3">
                    <WaitlistRowActions id={w.id} fromDateISO={isoDay(w.fromDate)} />
                  </td>
                )}
              </tr>
            ))}
