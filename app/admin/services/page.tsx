...
export default async function ServicesPage() {
  ...
  return (
    <AdminShell user={session?.email} can={can} locale={locale}>
      ...
      <div className="mt-8">
        <ServicesManager
          services={services.map((s) => ({ id: s.id, slug: s.slug, treatmentSlug: s.treatmentSlug, name: s.name, category: s.category, vatClass: s.vatClass, active: s.active, status: s.status, variants: s.variants }))}
