...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<{ range?: string }> }) {
  ...
  //
  // BLD-1126: goodsCost/usedCost/minMarginPct/vatCfg/usedRow/sessions each only
  // depend on the already-known since/bookingWhere, not on each other or on
  // staffRows above — batched into one Promise.all instead of six sequential
  // round trips.
  ...
  const [goodsCost, usedCost, minMarginPct, vatCfg, usedRow, sessions] = await Promise.all([
