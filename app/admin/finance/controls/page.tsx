...
export default async function FinancialControlsPage() {
  ...
  const [refundWindowDays, minMarginPct, vat, can, locale] = await Promise.all([getConfigNumber('refund_window_days'), getConfigNumber('min_margin_pct'), getVatConfig(), sessionPermissions(), getLocale()]);
  return (
    <AdminShell user={session?.email} can={can} locale={locale}>
      ...
      <div className="mt-8"><FinancialControls refundWindowDays={refundWindowDays} minMarginPct={minMarginPct} vat={vat} /></div>
    </AdminShell>
  );
}
