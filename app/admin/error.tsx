...
// PRJ-1200.11: pairs "Try again" (re-render) with a "Return to dashboard" escape
// route, so a crash loop doesn't strand a signed-in admin with no way out.
    ...
    <div className="grid min-h-screen place-items-center bg-[var(--color-bone)] p-6 text-center">
      <div className="max-w-md">
        ...
        <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
          <button onClick={reset} className="rounded-full bg-[var(--color-ink)] px-5 py-2.5 text-sm text-[var(--color-porcelain)]">Try again</button>
          <Link href="/admin" className="rounded-full border border-[var(--color-line)] px-5 py-2.5 text-sm font-medium text-[var(--color-ink)]">Return to dashboard</Link>
        </div>
      </div>
    </div>
  );
