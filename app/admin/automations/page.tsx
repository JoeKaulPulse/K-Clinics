...
const coreAutomations = [
  { name: 'Appointment reminder', desc: 'A gentle care reminder the day before a confirmed appointment, with a manage link.', kind: 'APPOINTMENT_REMINDER', trigger: '24 hours before visit', settingKey: null },
  ...
  { name: '3-day reminder', desc: 'An early heads-up 72 hours before a confirmed appointment — gives clients more time to prepare or reschedule. Enable in Settings.', kind: 'APPOINTMENT_REMINDER', trigger: '72 hours before visit', settingKey: 'reminder_72h' },
  { name: '2-day reminder', desc: 'A reminder 48 hours before a confirmed appointment. Useful alongside the standard 24h reminder. Enable in Settings.', kind: 'APPOINTMENT_REMINDER', trigger: '48 hours before visit', settingKey: 'reminder_48h' },
];
...
export default async function AutomationsPage() {
  ...
  const [counts, r72, r48] = await Promise.all([
    db.emailEvent.groupBy({ by: ['kind'], where: { status: 'SENT', createdAt: { gte: since } }, _count: { _all: true } }),
    getSetting('reminder_72h'),
    getSetting('reminder_48h'),
  ]);
  const countFor = (kind: string) => counts.find((c) => c.kind === kind)?._count._all ?? 0;
  const settingEnabled: Record<string, boolean> = { reminder_72h: Boolean(r72), reminder_48h: Boolean(r48) };
    ...
    <AdminShell user={session?.email} can={can} locale={locale}>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2">
        {coreAutomations.map((a) => {
          const isOptIn = a.settingKey !== null;
          const active = isOptIn ? settingEnabled[a.settingKey!] : true;
          return (
            <div key={`${a.kind}-${a.trigger}`} className="rounded-[var(--radius-lg)] border border-[var(--color-line)] bg-[var(--color-porcelain)] p-6">
              <div className="flex items-center justify-between">
                ...
                {active
                  ...
                  : <Link href="/admin/settings" className="rounded-full border border-[var(--color-line)] px-3 py-1 text-xs text-[var(--color-stone)] hover:border-[var(--color-gold)]">Off — enable in Settings</Link>
                }
              </div>
              ...
              <div className="mt-4 flex items-center justify-between text-xs text-[var(--color-stone)]">
                ...
                {active && <span>{countFor(a.kind)} sent / 30d</span>}
              </div>
            </div>
          );
        })}
