...
export async function eraseClientData(clientId: string) {
  ...
  await db.$transaction([
    ...
    // guest checkouts leave clientId null with only the order's own email column
    // set, so also match by email once it's known to be this client's address
    // (mirrors the GiftVoucher purchaserEmail fallback below, BLD-1715).
    db.order.updateMany({ where: { OR: [{ clientId }, { email: client.email }] }, data: { name: 'Erased', email: erasedEmail, phone: null, shipName: null, shipLine1: null, shipLine2: null, shipCity: null, shipPostcode: null } }),
