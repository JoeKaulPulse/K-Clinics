import { NextResponse } from 'next/server';
import { z } from 'zod';
import { crmEnabled } from '@/lib/crm';

export const runtime = 'nodejs';

const schema = z.object({ email: z.string().email(), password: z.string().min(1) });

export async function POST(req: Request) {
  if (!crmEnabled) return NextResponse.json({ ok: false, error: 'Accounts are not enabled.' }, { status: 503 });
  const parsed = schema.safeParse(await req.json().catch(() => ({})));
  if (!parsed.success) return NextResponse.json({ ok: false, error: 'Enter your email and password.' }, { status: 422 });
  const email = parsed.data.email.toLowerCase();
  // Brute-force protection with per-EMAIL lockout (parity with the admin/client
  // logins, which use loginGate; this route previously had only per-IP limiting,
  // so a distributed attack on a known trainee email was unthrottled). loginGate
  // enforces the 5-fail/15-min account lock + per-IP burst; recordLogin feeds the
  // failure counter that drives it. (BLD-402)
  const { loginGate, recordLogin, recordSecurity } = await import('@/lib/security/guard');
  const gate = await loginGate(email, req);
  if (gate.blocked) {
    await recordSecurity('RATE_LIMITED', 'academy', email, req);
    return NextResponse.json({ ok: false, error: 'Too many attempts. Please wait a few minutes and try again.' }, { status: 429 });
  }
  const { loginStudent } = await import('@/lib/academy-auth');
  const result = await loginStudent(parsed.data.email, parsed.data.password);
  await recordLogin('academy', email, Boolean(result.ok), req);
  return NextResponse.json(result, { status: result.ok ? 200 : 401 });
}
