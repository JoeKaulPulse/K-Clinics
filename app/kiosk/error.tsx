...
export default function KioskError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <main id="main" className="grid min-h-screen place-items-center bg-[var(--color-bone)] p-6 text-center">
      <div className="max-w-md">
        <p className="text-xs uppercase tracking-[0.16em] text-[var(--color-gold-deep)]">KClinics Kiosk</p>
        <h1 className="mt-2 font-[family-name:var(--font-display)] text-2xl">Something went wrong.</h1>
        <p className="mt-3 text-sm text-[var(--color-stone)]">An unexpected error occurred{error?.digest ? ` (ref: ${error.digest})` : ''}. Please ask a member of staff for help.</p>
