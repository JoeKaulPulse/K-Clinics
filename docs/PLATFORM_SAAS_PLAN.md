...
- **1c — `tenantId NOT NULL`.** `prisma/migrations/20260617190000_academy_tenant_not_null/` **self-backfills** any remaining NULLs to the default tenant (resolved from the `Tenant` table — portable) **then** sets `tenantId NOT NULL` on all 22 Academy tables, atomically. So it has **no precondition** — it can't fail on pre-existing NULLs and needs no prior cron run. `schema.prisma` made the field required; `tenantId` threaded through ~26 create sites (request-scoped via `currentTenantId()`, course-scoped in the cron content/exam-bank seeders, `student.tenantId` in learner writes). The now-obsolete Ring 0 backfill retired (`lib/tenant.ts` + the cron call removed).
- **Deploy hardening:** `scripts/db-sync.mjs` now skips schema sync on **any** non-production deploy (`VERCEL_ENV !== 'production'`) — for `db push` *and* `migrate deploy`. Preview builds point at the shared prod DB, so they must never mutate its schema; production is the single schema writer. (This fixes preview builds that were failing while trying to `db push` the 1c schema to prod.)
...
Sequencing: 1a → 1b shipped; 1c is self-sufficient (merge anytime; PITR snapshot still recommended). The live booking/admin schema is untouched throughout — only the Academy tables change.
