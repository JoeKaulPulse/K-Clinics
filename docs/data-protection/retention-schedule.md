...
| Cached answer translations | `HealthAssessmentTranslation` | Parent assessment | **With the parent assessment** — the row is keyed on `assessmentId` with `ON DELETE CASCADE`, so both the erasure flow (`app/admin/actions.ts`) and the 8-year purge above remove it automatically. No separate purge job needed. | Same as the assessment it derives from |
