...
### [Critical] ✅ REMEDIATED — Right-to-erasure leaves health and personal data behind (pseudonymises only the Client row)

> **Remediated 2026-06-12.** `eraseClientData` (`app/admin/actions.ts:31-82`) is now one atomic `$transaction` covering every table named in the Issue below: it pseudonymises the `Client` row, strips special-category free-text from the financially-retained `Booking`/`Consultation` rows, deletes `ConsultationNote` via its `consultationId` link (the Client-cascade can't reach it — added this pass), hard-deletes `Interaction`/`HealthAssessment`/`BeforePhoto`/`AiAnalysis` (→ `AiAnalysisImage` by FK cascade)/`SignedConsent`/`Review`/`NpsResponse`/`FollowUp`/`EmailEvent`/`AppointmentSession`, and scrubs `CallRecord` transcript/recording/raw. *Residual (Medium, tracked as a follow-up card): extend to non-special-category personal data on `Referral`, `ChatConversation`/`ChatMessage`, `WaitlistEntry`, legacy `Appointment`.* Original finding retained below for context.
...
### [Critical] ✅ REMEDIATED — Special-category (health) data stored in plaintext outside the encrypted store

> **Remediated 2026-06-12.** All six fields now route through `lib/clinical-crypto.ts` (`encClinical`/`decClinical`) over the existing AES-256-GCM keyring: encrypt-at-write at every entry point (`/api/admin/medical-flag`, `/api/booking/start`, `/api/consult`, `app/admin/clients/actions.ts`, `app/admin/actions.ts` interaction notes) and tolerant decrypt-at-read in the `lib/crm-data.ts` access layer plus each direct read site. `decClinical` passes legacy plaintext through unchanged, so there was no flag-day; `lib/clinical-crypto-backfill.ts` (+ `/api/admin/maintenance/backfill-clinical-encryption`) upgrades historic rows. The last missed read-site — the live appointment-session canvas rendering `medicalFlag` raw to clinicians (`app/admin/bookings/[id]/session/page.tsx`) — was fixed this pass. Values become ciphertext so they can no longer be SQL-`contains` searched; admin search/calendar/my-day use presence-only badges, which still work. Original finding retained below for context.
