  ...
  const groups: GoLiveGroup[] = [
    ...
    },
    {
      ...
      items: [
        ...
        mk('chat-inbound', has(env.CHAT_INBOUND_DOMAIN), { title: 'Live-chat email replies (Resend Inbound)', what: 'Lets a visitor who left their email reply by email — it threads back into the same chat.', optional: true, owner: true, how: ['In Resend → Inbound, add the inbound domain mail.' + host + '.', 'In Cloudflare DNS, add the CNAME and CAA records Resend gives you for mail.' + host + ', then click Verify in Resend.', 'Point the Resend inbound webhook at https://' + host + '/api/webhooks/chat-inbound.', 'Set CHAT_INBOUND_DOMAIN=mail.' + host + ' and RESEND_INBOUND_SECRET in the environment.', 'To test before the domain verifies, set CHAT_INBOUND_DOMAIN to the Resend sandbox inbound domain (…​.resend.app).'] }),
      ],
    },
    {
