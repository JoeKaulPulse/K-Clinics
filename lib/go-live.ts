...
export async function goLiveChecklist(): Promise<GoLiveState> {
  ...
  const groups: GoLiveGroup[] = [
    ...
    },
    {
      ...
      items: [
        mk('gbp', gbpLocation, { title: 'Google Business Profile (reviews + manage)', what: 'Imports Google reviews and lets you reply to them and manage the profile from the dashboard.', owner: true, links: [EXT.googleCloud, EXT.gbp, { label: 'Marketing → Connections', href: '/admin/marketing/connections' }], how: [
          ...
          '5 · Claim/verify the clinic at business.google.com (if not already), then in the dashboard go to Admin → Reviews and click “Connect Google Business” — approve the business.manage scope. (This is on the Reviews page, not Marketing → Connections — that one is for Google Ads/Analytics.)',
          '6 · On connect we auto-detect your account + location and reviews start importing (reply/manage under Admin → Reviews). Only if you have several locations and need to pin one: set GOOGLE_BUSINESS_ACCOUNT_ID + GOOGLE_BUSINESS_LOCATION_ID.',
          '7 · Validate: this item flips to “Ready” automatically once connected, and your reviews appear under Admin → Reviews (use “Sync now” to pull immediately). If the list is empty or a Google 403 shows, the Business Profile API access grant from step 1 is still pending Google’s approval (can take a few days) — the connection is fine, the data just isn’t released yet.',
        ] }),
    ...
    },
    {
      ...
      items: [
        mk('sms', has(env.TWILIO_ACCOUNT_SID) && has(env.TWILIO_AUTH_TOKEN), { title: 'SMS reminders (Twilio)', what: 'Text reminders & booking confirmations.', optional: true, owner: true, links: [EXT.twilio, EXT.vercelEnv], how: ['Create a Twilio account & buy a number.', 'Set TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN (the sending number is built in).'] }),
