  ...
  const groups: GoLiveGroup[] = [
    ...
    },
    {
      ...
      items: [
        ...
        mk('chat-inbound', has(env.CHAT_INBOUND_DOMAIN), { title: 'Live-chat email replies (Resend Inbound)', what: 'Lets a visitor who left their email reply by email — it threads back into the same chat.', optional: true, owner: true, how: ['In Resend, enable Inbound on a reply subdomain (e.g. reply.' + host + ').', 'Add the MX records it gives you in Cloudflare.', 'Point the inbound webhook at …/api/webhooks/chat-inbound.', 'Set CHAT_INBOUND_DOMAIN (the reply subdomain) + RESEND_INBOUND_SECRET.'] }),
      ],
    },
    {
