...
export async function runDailyAutomations(): Promise<Tally> {
  ...
  const settled = await Promise.allSettled([birthdays(t), followUps(t), reviews(t), winBacks(t), reminders(t), formReminders(t), treatmentFollowUps(t), scheduledGiftVouchers(t), tierNudges(t), anniversaries(t), abandonedBookings(t), membershipRenewal(t), staffWeeklyDigest(t), staffReengagement(t), keyReencryption(t)]);
  for (const r of settled) {
    if (r.status === 'rejected') { t.errors++; console.error('[automations] unexpected throw:', r.reason); }
  }
  return t;
}
