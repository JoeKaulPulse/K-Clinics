...
import { K_MARK_LIGHT_B64, K_LOCKUP_B64 } from '@/lib/brand-email-assets';
...
// A sent email carries the header brand lockup as an inline cid: attachment, which
// a browser can't resolve — so for the dashboard preview we swap the cid refs for
// equivalent data: URIs (the same bundled bytes) so the real marks show here too.
const forPreview = (html: string) =>
  html
    .replace(/cid:klockup/g, `data:image/png;base64,${K_LOCKUP_B64}`)
