...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  },
  {
    ...
    notes: [
      ...
      'Review fix (audience): the first draft mailed every client with termsAcceptedAt null and pointed them at /account/login to "accept our Terms & Conditions and add a payment card". Neither action exists there: acceptance is recorded only by the signup tick (registerClient stamps it even when the Client row already exists), the portal booking route /api/booking/start does not record it at all, and no screen in /account adds a card -- Stripe saves one during a booking. A client who already holds a portal password therefore had no way to clear the flag and would have been asked for ever. The query now also requires passwordHash: null, the CTA goes to /account/signup, and the copy says what actually happens with the card instead of asking for it.',
