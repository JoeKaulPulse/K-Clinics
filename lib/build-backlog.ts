...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  },
  {
    ...
    notes: [
      ...
      'Fix (BLD-1309, job applications): app/api/cron/daily/route.ts now reads the cvUrl of every JobApplication row about to be purged by the existing 6/12-month retention sweep, and calls Vercel Blob del() on those URLs before the deleteMany runs -- mirroring lib/kiosk.ts\'s deleteKioskBlobs pattern. Best-effort: a Blob delete failure is logged and does not block the DB purge.',
