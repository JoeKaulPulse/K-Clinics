...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  },
  {
    ...
    notes: [
      ...
      'Review fix (pre-merge, BLD-1436): the first cut regressed the one field that already had a live inline error. dobMsg was computed as `(f.dob ? dobError(f.dob) : null) || errors.dob`, so once a submit with an empty date of birth had written errors.dob, picking a valid date left dobError() returning null and the stale "Date of birth is required." showing (with aria-invalid still set) until the next submit — where before the change it cleared as soon as the field was valid. dobMsg now falls back to errors.dob only while f.dob is still empty. The same class of staleness applied to the six new fields, so every signup input\'s onChange also clears its own key via a clearErr() helper, and setting field errors now clears the generic banner so a network error from a previous attempt can\'t sit underneath the new inline messages.',
      'Review fix (pre-merge, BLD-1436): the inline messages carried aria-invalid but no aria-describedby, so a screen reader announced "invalid entry" on the field without ever reading which rule failed. Each message now has an id and each input points at it while the error is showing, including the date-of-birth field that had the same gap before this batch.',
  ...
  },
  {
    ...
    notes: [
      ...
      'Every fetch() call site in the three files (5 in lib/xero.ts, 5 in lib/google-calendar.ts, 7 in lib/google-business.ts -- OAuth token exchanges, the Xero Accounting API reads/writes and Bank Summary report, the Google Calendar events sync/push/delete, and the Google Business accounts/locations/reviews/posts/reply calls) now goes through fetchWithRetry() with a per-integration label for the retry log line. Existing status handling (403 permission prompts, 404 stale-event recreation, 429/403 treated as pending Business Profile access) is unchanged -- fetchWithRetry returns the same Response shape as fetch, so callers that inspect res.status/res.ok needed no other changes.',
      'Retry safety at the write call sites was reviewed one by one. The Google Calendar PATCH, the Google Business review-reply PUT and its DELETE are all idempotent, so a retry re-sends the same end state. The Xero writes are not: POST /Invoices, /Payments, /CreditNotes and /Contacts each create a new record per call, so a retry after a 5xx or a timeout on a request Xero had already committed would double-invoice. xeroWrite() now sends a per-call Xero Idempotency-Key header (one uuid generated per logical write, shared by all attempts of that write), which makes a replayed attempt return the original response instead of writing again.',
      'Residual, accepted: the first push of a booking to a clinician calendar is a POST with no client-supplied event id, so a retry after an ambiguous timeout can leave a duplicate "KClinics appointment" event (tagged kcBookingId, findable) while only the last id is stored. Same class of duplicate-send risk BLD-1038 already accepted for email/SMS, and cheaper to live with than the 409-conflict handling a deterministic event id would need.',
  ...
  {
    title: 'Stripe webhook timeout risk, skip-to-content link check, stale no-show fee email, and dentistry structured data overclaiming availability (BLD-1479, BLD-1481, BLD-1482, BLD-1483)',
    type: 'ERROR', urgency: 'P1', status: 'IN_REVIEW', assignee: 'claude',
    value: 6, effort: 2,
    detail: 'End-of-Day Audit findings, 24 Aug: BLD-1479: app/api/stripe/webhook/route.ts had no maxDuration despite awaiting finalizeBookingCharge/finalizeOrder/confirmVoucher, each of which awaits sendEmail() -- lib/email.ts\'s acquireSendSlot() can poll up to 30s under Resend\'s 5/sec cap, risking a platform-default timeout mid-transaction on the most financially-critical endpoint. BLD-1481: reported as components/layout/Header.tsx having no "Skip to main content" link, forcing keyboard users to tab through the full nav on every page. Not reproducible -- app/(marketing)/layout.tsx already renders that link above the header. Closed with no code change; see the note below. BLD-1482: app/admin/bookings/actions.ts sent the "Sorry we missed you" no-show email using booking.chargedPence read BEFORE applyNoShowFee() ran, so the email always reported the fee as unpaid even when one had just been charged. BLD-1483: app/(marketing)/[slug]/page.tsx called serviceLd() with a live price for dentistry treatments even when dentistryLive is false, so the JSON-LD claimed InStock availability and a bookable price for a service the clinic cannot yet deliver, while the on-page meta already switched to Coming Soon framing.',
    notes: [
      'BLD-1479: added export const maxDuration = 60 to the webhook route, matching the 60s convention already used on the kiosk analyze/photo routes, cron/dispatch and admin/api-health. Safe against Stripe\'s own ~30s response timeout because the route already claims each event id in the ProcessedStripeEvent ledger (BLD-714/PRJ-1043.2) and every handler is CAS-idempotent, so a redelivery is a no-op rather than a double charge.',
      'BLD-1481: WITHDRAWN as a false positive at review. app/(marketing)/layout.tsx line 30 already renders <a href="#main">Skip to content</a>, above AnnouncementBar and Header, so it is already the first focusable element in the document on every marketing page. The audit read Header.tsx and the <main id="main"> landmark but missed the existing link ten lines above. A second copy inside the header was written and then reverted: it would have added a duplicate tab stop with identical text, target and position, and app/not-found.tsx renders this Header outside the marketing layout with no <main id="main"> at all, so the header copy would have been a dead anchor there. Header.tsx now carries a comment recording why no skip link belongs in it, so the next audit pass does not re-raise this.',
      'BLD-1482: moved the rebooking-note email send in setBookingStatus (NO_SHOW branch) to after applyNoShowFee() runs, and pass its returned `charged` amount as feePence instead of the stale pre-fee read. applyNoShowFee returns charged: 0 on every branch that took no card fee (waived, prepaid package session consumed, alreadyPaid, 3DS requiresAction, declined), so tmplNoShow\'s "a fee of £X was applied" paragraph is correctly omitted for all of them and only appears on a genuine successful charge. If applyNoShowFee itself throws, fee is null and no fee amount is sent: b.chargedPence is the booking\'s total settled charge, not a no-show fee, so falling back to it would tell an already-paid client a fee had been taken when none was. The setting check, dedupe check, gift-voucher release and staff notification are unchanged; the voucher release still runs before the fee, so the BLD-882 pre-fee chargedAt guard is untouched.',
      ...
      'Open gap (separate ticket, deliberately NOT fixed here): app/not-found.tsx renders Header outside the marketing layout and has no <main> landmark and no skip link of its own, so the 404 page is the one public page with neither. Left alone because the fix is a page-structure change to not-found.tsx rather than anything in this batch, and it needs its own check against the root layout.',
  ...
  {
    title: 'Client colour status (green/yellow/red) set from Close Booking, shown on calendar/profile/booking and blocking RED online (BLD-1532)',
    type: 'TASK', urgency: 'P1', status: 'SHIPPED', assignee: 'claude',
    value: 8, effort: 5,
    detail: 'Owner request via info@kclinics.co.uk: staff need to mark a client GREEN (good, books normally), YELLOW (caution, still bookable, visible to staff) or RED (blocked from booking online, staff warned) from the booking detail page, with the status persisting on the client profile until changed, and visible on the calendar, client profile and booking creation.',
    notes: [
      'Schema (additive, no @unique): Client gained clientStatus ClientStatus? / clientStatusSetBy String? / clientStatusAt DateTime? / clientStatusReason String?, and a new ClientStatus enum (GREEN/YELLOW/RED), mirroring the existing medicalFlag/patchTestResult pattern. AuditAction gained CLIENT_STATUS_CHANGED. Migration: prisma/migrations/20260828000000_client_status/migration.sql (CREATE TYPE, ALTER TYPE ADD VALUE, four nullable ADD COLUMNs -- confirmed via prisma migrate diff, no drops, safe under prisma db push without --accept-data-loss).',
      'app/api/admin/client-status/route.ts: new POST route (mirrors app/api/admin/patch-test/route.ts) gated on clients.edit only (not clinical.view -- this is not clinical data), validates GREEN/YELLOW/RED/null, writes the four fields and logs CLIENT_STATUS_CHANGED via logAudit.',
      'components/admin/ClientStatusEditor.tsx: 3-way pill picker (green/yellow/red), same interaction weight as MedicalFlagEditor/PatchTestEditor. Green saves immediately; yellow/red pause for an optional free-text note before saving. components/admin/ClientStatusBadge.tsx exports ClientStatusBadge (pill, used on the client profile header and the booking picker) and ClientStatusDot (a bare coloured dot for the calendar) -- both render nothing for GREEN/null, so the default case stays visually quiet and only yellow/red stand out, per the ticket.',
      'Setter surfaced on the booking detail page (app/admin/bookings/[id]/page.tsx), inside the Client card next to the Close Booking actions -- the page the ticket names. Also surfaced on the client profile (app/admin/clients/[id]/page.tsx) next to MedicalFlagEditor/PatchTestEditor, since the ticket says the status must persist and be editable from the profile too.',
      'Display surfaces: (1) calendar (app/admin/calendar/page.tsx) -- a coloured dot before the client name on each appointment block for yellow/red only. (2) client profile -- a badge next to the client name in the page header, plus a red banner when blocked. (3) booking detail page -- a badge next to the treatment title, plus a red banner for staff managing/closing an existing booking for a RED client. (4) new-booking creation picker (components/admin/NewBookingButton.tsx, backed by searchClientsForBooking in app/admin/bookings/create-action.ts) -- a badge on each search match and the selected client, plus a warning (not a block) when the selected client is RED, since staff-initiated bookings are explicitly allowed to proceed with a warning per the ticket.',
      'Public-side hard block: app/api/booking/start/route.ts (the live account-based booking flow BookingFlow.tsx actually calls) now rejects with 403 before any slot is held or Stripe customer/SetupIntent is touched when the signed-in client is RED. app/api/booking/create/route.ts (an older unauthenticated public endpoint with no current callers in the app, upserts a client by email) got the same check for defence in depth, since it is still a public path that could otherwise sidestep the block.',
      'Palette: GREEN uses --color-jade (existing success/active token); RED uses --color-blush / --color-blush-deep (existing destructive/error token); YELLOW has no dedicated theme token, so it follows the codebase\'s existing precedent of Tailwind amber-100/amber-500/amber-800 for warning-weight UI (already used for the PENDING booking badge and the FOLLOW_UP note style) rather than inventing a new brand colour.',
      'Review fix: the new status dot made the calendar\'s name line a flex container while leaving truncate on that same element -- text-overflow never applies to a flex item, so a long client name hard-clipped and could overflow the block. Moved truncate to the text child.',
      'Review: confirmed the RED-block cannot misfire against GREEN/YELLOW/null clients (strict === \'RED\' check against a fully-loaded record, not a narrow select). Known follow-up, not a defect: a RED client can still book under a different email (guest checkout upserts by email, same limitation as the existing unpaid-balance gate), and /api/booking/reschedule is token-based and not gated on status.',
      'npx prisma generate run after the schema edit. Verified: npx tsc --noEmit and npm run build pass clean (build run with DB_SYNC_NONFATAL=true -- this sandbox has no raw Postgres TCP egress to the Neon host at all, confirmed with a direct connection test, unrelated to this change; the schema is additive-only so the real deploy-time prisma db push gate is unaffected).',
    ],
  },
  {
  ...
  },
  {
    ...
    type: 'ERROR', urgency: 'P2', status: 'SHIPPED', assignee: 'claude', pr: PR(1876),
    value: 5, effort: 2,
  ...
  },
  {
    ...
    type: 'ERROR', urgency: 'P2', status: 'SHIPPED', assignee: 'claude', pr: PR(1877),
    value: 6, effort: 2,
    ...
    notes: [
      ...
      "Review correction (pre-merge), stored-value compatibility: businessDate is not just a window bound, it is an equality KEY. Existing rows hold server-UTC midnight (00:00Z); localDayStart() now returns clinic midnight, which during BST is 23:00Z on the previous date, so getDayClose() and the ?status=1 count would have stopped matching every close recorded during BST. Effect on the deploy day: the reminder banner tells staff the day is unclosed although it was closed hours earlier, and a POST finds no existing row and writes a SECOND DayClose for a day that already has one (there is no unique constraint, and the deploy gate forbids adding one). Both lookups now match over the clinic-day window (gte dayStart, lte dayEnd) rather than an exact instant. Consecutive windows are contiguous and 24h wide, so a stored instant still lands in exactly one clinic day and no cross-day collision is possible. POST rewrites businessDate to the new instant, so touched records self-heal.",
      "Review correction (pre-merge), rendering: moving businessDate to 23:00Z-previous-date during BST also broke the two places that formatted it without a timeZone. app/admin/day-close/reports/page.tsx is a server component (force-dynamic) on a UTC host, so every summer close-out would have been listed under the previous day's date; components/admin/DayCloseRunner.tsx formats in the device timezone, wrong for any staff browser not set to Europe/London. Both now use fmtClinicDate() from lib/clinic-time.ts, which is what that module's own header requires of anything rendering a clinic time.",
      'Known and accepted: clinicDayBounds() defines dayEnd as dayStart + 24h - 1ms, so on the two DST transition days the window overlaps the next clinic day by an hour (spring) or stops an hour short (autumn). Both fall between 23:00 and 01:00 local with the clinic shut, and this is existing shared-helper behaviour that the availability engine and calendar already rely on -- changing it is a separate piece of work, not part of this fix.',
      'Verified: npx tsc --noEmit and npm run build pass clean.',
    ],
  },
  {
    title: 'Plain --color-gold text fails WCAG AA contrast on marketing and admin pages (BLD-1488, BLD-1462)',
    type: 'TASK', urgency: 'P2', status: 'SHIPPED', assignee: 'claude', pr: PR(1878),
    value: 6, effort: 2,
  ...
  },
  {
    ...
    notes: [
      'Fix: exported bookingSpendPence() (previously private) and changed reverseSpendPoints() to accept the booking-shaped object (chargedPence, giftVoucherPence, chargePaymentIntentId) instead of a raw chargedPence number, computing spend = bookingSpendPence(b) as the ratio denominator -- exactly matching the earn-side calculation.',
      'The voucher-covered portion is only itself restored to the client when the card portion is refunded IN FULL (booking-actions.ts / the webhook only credit the voucher back when `fully` is true) -- so the fix folds the voucher slice into the numerator (effectiveReturnedPence) only once totalRefundedPence has reached chargedPence, keeping a full refund at ratio 1.0 (unchanged behaviour) while correcting the ratio for any partial refund. A booking with no voucher (voucherPortion = 0) is mathematically unaffected -- no regression for the common non-voucher case.',
      'Updated all 3 call sites (lib/booking-actions.ts refundBooking, app/api/stripe/webhook/route.ts dispute-lost and charge.refunded handlers) to pass the full booking record instead of booking.chargedPence -- each already had giftVoucherPence and chargePaymentIntentId selected/available.',
  ...
  },
  {
    ...
    notes: [
      ...
      "Review follow-up (BLD-1523): the first pass validated the pounds figure (n > 0) but sent Math.round(n * 100) unchecked, so 0.004 passed the client check, rounded to 0p and came back from the API as a bare 'Bad request' alert. Validation now runs on the rounded pence value (must be >= 1p). The input also carries aria-invalid and aria-describedby pointing at the role=\"alert\" message.",
      "Review follow-up (BLD-1543): the 45vh cap now applies only before 'Customise' is opened; with the three toggle rows showing it relaxes to 80vh below sm, so 'Save choices' is not pushed behind the banner's own scrollbar (the BLD-1355 failure). sm: and up are unchanged.",
      'Verified: npx tsc --noEmit and npm run build pass clean.',
    ],
  },
  {
    title: 'Non-constant-time webhook sigs, unscoped homework uploads, admin search privacy, Calendar button gating, scoped font preload (BLD-1522, BLD-1544, BLD-1541, BLD-1460, BLD-1463)',
    type: 'TASK', urgency: 'P2', status: 'IN_REVIEW', assignee: 'claude',
    value: 5, effort: 2,
    detail: "BLD-1522: app/api/webhooks/resend/route.ts and app/api/webhooks/chat-inbound/route.ts verified the Svix signature with a plain === string comparison instead of a constant-time one, unlike every other secret comparison in the codebase (lib/kiosk.ts, lib/cron-auth.ts, lib/oauth-state.ts, app/api/build/queue/route.ts). BLD-1544: app/api/academy/homework/blob-token/route.ts's onBeforeGenerateToken returned an unrestricted upload token for any client-requested pathname once a student was authenticated, unlike the sibling portfolio/blob-token route, which restricts uploads to a portfolio/ prefix. BLD-1541: components/admin/GlobalSearch.tsx wrote every raw searched term (patient names/numbers) to localStorage indefinitely, with no cap and no clearing on logout. BLD-1460: the Google Calendar OAuth start route (app/api/admin/gcal/connect/route.ts) was gated on a separate 'parked' flag (googleEnabled()) from the one the Connect button on /admin/schedule checks (googleConfigured()), so a configured-but-parked deployment sent staff to a raw JSON error page instead of the page's existing inline banner. BLD-1463: app/layout.tsx applied the GeistMono font variable on <html> in the root layout, so Next.js preloaded its ~71KB woff2 on every route, including the public marketing homepage, even though it is used almost exclusively in /admin and /account.",
    notes: [
      'BLD-1522: replaced sig.split(\' \').some((s) => s.split(\',\')[1] === expected) with a per-candidate crypto.timingSafeEqual compare (length-checked first, so a mismatched length never throws), in both app/api/webhooks/resend/route.ts and app/api/webhooks/chat-inbound/route.ts.',
      "BLD-1544: app/api/academy/homework/blob-token/route.ts's onBeforeGenerateToken now reads the requested pathname and throws unless it starts with 'academy/homework/' -- the exact prefix components/academy/HomeworkPanel.tsx already uploads to -- mirroring the portfolio route's portfolio/ check.",
      "BLD-1541: components/admin/GlobalSearch.tsx now stores only the destination a search actually navigated to (result title/sub/href, capped at the last 6), never the raw typed query text; clicking a recent entry navigates straight to it instead of re-populating the search box. The recent-search key is also cleared in AdminShell.tsx's signOut() so it does not persist across a staff logout on a shared workstation.",
      "BLD-1460: app/api/admin/gcal/connect/route.ts's googleEnabled() gate now redirects to /admin/schedule?gcal=error (the existing inline banner the OAuth callback route already uses on failure) instead of returning a raw JSON 503.",
      'BLD-1463: removed GeistMono from app/layout.tsx (root <html> class). It is applied instead inside app/admin/layout.tsx and app/account/layout.tsx (each on their own wrapper div, since only the root layout can set classes on <html>/<body>), so the font is preloaded only on the routes that actually render it.',
      'Review fix (BLD-1463): the claim that --font-mono still chained to ui-monospace/monospace on public routes was wrong. --font-mono was declared as var(--font-geist-mono), ui-monospace, monospace; with --font-geist-mono undefined and no inner fallback, the var() substitution fails and poisons the whole custom property, so every `font-mono` consumer outside /admin and /account became invalid at computed-value time and silently inherited the body sans face. That is wider than the four pages listed above -- it also hit components/kiosk/ClaimReward.tsx (the kiosk reward code), components/gift/GiftVoucherFlow.tsx (the voucher code) and components/academy/DemoPlayer.tsx -- i.e. exactly the codes a user has to read back character by character. Fixed in app/globals.css by declaring --font-mono: var(--font-geist-mono, ui-monospace), ui-monospace, monospace, so public routes get the real system monospace with no webfont download and the /admin + /account preload saving is kept.',
      'Review fix (BLD-1541): storing the destination title/sub was not by itself a privacy fix -- /api/admin/search labels a client hit with the full name plus the medical-flag marker and its sub with the email address, a booking sub with the client name, and a consultation sub with a snippet of the DECRYPTED clinical concerns, so the change moved more identifying data into localStorage than the raw query it replaced. components/admin/GlobalSearch.tsx now persists a hit only when its group is in RECENT_SAFE_TYPES (nav, services, products, stock, courses, posts, pages, tasks, build, vacancies, discounts); client/booking/consultation/review/staff/student/application/supplier/voucher hits are never remembered.',
      'Review fix (BLD-1541): the localStorage value changed shape from string[] to {title, sub, href}[] without a migration, so any admin with the old format would have rendered blank recent rows that navigate nowhere. The loader now validates each entry and rewrites (or removes) the key, which also purges the old raw-query entries on first load.',
  ...
  },
  {
    ...
    notes: [
      ...
      'Review fix (adversarial pass), two things the first cut would have shipped wrong the day the owner switched it on. (1) Per-booking dedup alone does not bound how often a PERSON is asked, and a course client is the normal case here rather than an edge one -- laser hair removal runs 6-8 sessions, often weekly, each its own bookingId -- so the same client would have been asked to refer a friend every week for the length of their course. Added a 90-day per-client cooldown alongside the per-booking dedup, in the same spirit as the 30-day tier-nudge and 120-day membership-renewal dedups. (2) The email said both sides would be "thanked with loyalty points once they complete their first treatment", which understates the offer and omits its qualifying condition: the real terms (LOYALTY in lib/client-loyalty.ts, and the published /refer-a-friend page) are 2,500 points -- £25 -- to each side, and only when the friend\'s first treatment is £100 or more. Copy now states both, and derives the figures from LOYALTY so it cannot drift from the mechanism that pays out.',
      'Ships OFF by default (referral_ask_email: false) pending explicit owner sign-off to enable, matching the existing convention for new customer-messaging automations in this codebase (abandoned_order_recovery, nps_survey, tcs_reminder_email). Because the audience is bounded to a rolling 5-10 day window of completed visits, switching it on cannot retro-mail a backlog: the first run reaches only the last week or so of visits.',
      'Verified: npx tsc --noEmit and npm run build pass clean.',
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      ...
      'Enabled: flipped tcs_reminder_email to true in lib/settings.ts (BLD-1653 is the owner-filed go-ahead this was gated on since BLD-1452). The guards documented in the code comment (passwordHash-null audience, 3-attempt cap, 14-day cadence, claim-before-send) were re-checked against the code, not just the comment, and hold. Confirmed the audience does not include anyone who has already accepted: termsAcceptedAt is stamped by signup (lib/client-auth.ts), website booking (app/api/booking/create) and enquiry (app/api/consult), all first-acceptance-wins.',
      'Review fix (adversarial pass): the audience now also excludes clients tagged likely-test. scanAndTagTestClients (lib/test-clients.ts) and the WordPress migration tag junk/keyboard-mash signups with that tag and the admin client list hides them by default, and those rows match this audience exactly -- legacy, no portal password, no recorded acceptance, an address nobody reads. Every other automation that could reach them is marketing-class, so marketingOptIn/marketingConsentAt filters them out for free; this is the one care-class send with no such gate, so without the tag filter they would have been the first thing a 500-a-day run mailed, bouncing off the clinic sending domain and taking real booking-confirmation deliverability with them.',
      'Two things for the owner to check rather than code fixes. (1) SETTING_DEFAULTS only applies when no Setting row exists, so if this toggle was ever switched off in /admin/settings the deploy will not turn it on -- confirm "T&Cs acceptance reminder emails" reads on there after release. (2) Academy trainees get a Client row (source academy-signup, lib/academy-auth.ts) with no portal password and no recorded acceptance, so they are in the audience and will get a clinic-treatment-framed email. Not filtered here because whether a trainee counts as a client to ask is the owner\'s call, not a code decision.',
      'Verified: npx tsc --noEmit and npm run build pass clean.',
    ],
  },
  {
    ...
    notes: [
      'Fix: components/admin/dashboard/ClinicianView.tsx (the practitioner-role dashboard, already rendered instead of the full admin overview -- app/admin/page.tsx skips every clinic-wide query for non-admin views via lib/dashboard-views.ts\'s role-to-view resolver, which a Specialist cannot override) had one remaining leak: its "Whole clinic today" section fetched and rendered every practitioner\'s bookings/clients, gated only on the bookings.view permission -- which PRACTITIONER holds by default. canAllBookings is now additionally gated on session.role !== \'PRACTITIONER\', so the clinic-wide query never runs for a real Specialist session (an OWNER/ADMIN previewing the clinician view is unaffected, since that reflects their own real access).',
      'Fix: app/admin/calendar/page.tsx built the full multi-column calendar (every clinician + an Unassigned column, all bookings, all staff time-off, clinic-wide month counts) for any session with calendar.view, with no role or practitioner-id scoping at all. For a PRACTITIONER-role session (isSpecialist), every query is now filtered at the Prisma where-clause: the clinician roster query returns only their own adminUser row, booking.findMany (day diary and month-count queries) filters practitionerId: session.sub, and staffTimeOff.findMany filters staffId: session.sub. The rendered columns list drops the Unassigned column and every other practitioner\'s column entirely. OWNER/ADMIN/FRONT_DESK sessions are unaffected and still see the full calendar.',
      'Both fixes filter in the query itself, not by trimming an already-fetched result client-side, so a Specialist cannot recover clinic-wide stats or another practitioner\'s calendar by hitting these pages directly or refreshing. session.sub is the AdminUser id, which is the same id stored as Booking.practitionerId and StaffTimeOff.staffId, matching the scoping pattern already used elsewhere (app/admin/my-day/page.tsx, and ClinicianView\'s own "mine" query).',
      'Found on the direct-request check (owner\'s item 3): the "Rooms" widget ClinicianView renders (live-polled by components/admin/rooms/RoomPrepStatus.tsx, GET /api/admin/rooms/prep) returned every room\'s current/next occupant -- client name + treatment -- clinic-wide, for any session holding rooms.prep.manage, which PRACTITIONER holds by default. This is exactly the kind of leak the ticket describes and is reachable both on the dashboard\'s initial render and via the browser\'s own 20s poll, so it needed the same fix. lib/room-prep.ts getRoomsForDay() now takes an optional practitionerId that redacts current/next to null for any room whose booking is not that practitioner\'s (room prep/occupied status itself stays visible to everyone -- a Specialist still needs to see which rooms are free -- only the client-identifying booking details are scoped). Both call sites a Specialist can reach -- ClinicianView.tsx and the GET handler in app/api/admin/rooms/prep/route.ts -- now pass session.sub as practitionerId when session.role === \'PRACTITIONER\'. FRONT_DESK/OWNER/ADMIN callers (ReceptionistView.tsx, app/admin/page.tsx\'s admin-view "Rooms today") are untouched since front-of-house and admin genuinely need the whole clinic\'s room occupancy.',
      'Review fix (found on the adversarial pass, not in the first cut): app/admin/my-day/page.tsx had the same clinic-wide "Whole clinic" booking list on its own separate query, gated on bookings.view alone. It is not an unrelated page -- My Day renders this very ClinicianView on today and is the target of the widget\'s own "Open My day" link, so a Specialist could click through and press the previous-day arrow to get back every practitioner\'s appointments, client names and medical flags that the dashboard and calendar had just been scoped to withhold. canViewAll there is now gated on session.role !== \'PRACTITIONER\' as well, so the clinic-wide query never runs for a Specialist. Other bookings.view holders (OWNER/ADMIN/FRONT_DESK/STAFF) are unaffected.',
      'Known residual, not changed here because it is a product decision for the owner: PRACTITIONER still holds bookings.view and clients.view/clients.clinical.view by role default (lib/permissions.ts), so a Specialist can still open /admin/bookings and /admin/clients and read any client record. This ticket scoped the dashboard, calendar, My Day and room-prep surfaces it named; narrowing the role itself would change what a practitioner can do clinically and needs the owner to decide.',
      'Known residual: a booking with no practitionerId (auto_assign_practitioner off, or pickPractitioner finding nobody free) is now invisible to a Specialist on both the calendar and the dashboard -- the Unassigned column is deliberately not shown to them. The setting defaults on, so most bookings are assigned, but the owner should confirm assignment is actually happening before relying on a Specialist\'s diary being complete.',
      'Verified: npx tsc --noEmit and npm run build pass clean.',
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      'Storage decision: the codebase already has a genuine private-storage pattern for exactly this class of data -- BLD-798 (kiosk selfies) and BLD-740 (academy portfolio clinical photos) both upload to Vercel Blob with access:\'private\' and read back only through an authenticated per-feature relay route (never a public/unauthenticated URL). This is the strictest existing pattern in the app and was reused as-is for VTCT documents: new lib/vtct-blob.ts (putVtctBlob/fetchVtctBlob, mirroring lib/kiosk-blob.ts and lib/portfolio-blob.ts) plus an authenticated relay at /api/academy/vtct-registration/document that only serves the owning trainee or staff holding settings.manage -- everyone else gets an identical 404, matching the portfolio photo relay\'s anti-probing behaviour. This did NOT hit the ticket\'s STOP condition (no existing private pattern) -- the opposite was true, so building on it was the instructed path, not a new invention.',
      'Two caveats flagged for the owner, both inherited from the existing pattern rather than introduced here: (1) BLD-1304 found the connected production Blob store is currently provisioned public-only, so every access:\'private\' put() -- kiosk, portfolio, and now VTCT documents -- fails until the store is re-provisioned (Vercel dashboard -> Storage -> Blob -> private, or create a new private store and reconnect it). Until then, document upload in this new form will fail closed with an error rather than falling back to public storage -- the same deliberate trade BLD-1304 already made for kiosk photos, not a new regression. (2) The installed @vercel/blob client SDK cannot let the server pin a client upload token\'s access level (only pathname/content-type/size -- verified against the installed type defs), so a crafted client request could in principle still create a PUBLIC blob even once the store is private; BLD-740 already accepted this same residual gap for portfolio photos with a self-healing daily sweep. The same sweep now exists for VTCT documents (migrateVtctDocumentsIfNeeded in lib/vtct-blob.ts, wired into the daily cron alongside migratePortfolioPhotosIfNeeded) as defence in depth, since these are identity documents.',
      'Schema (additive only, no @unique added -- see lib/task-refs.ts\'s pattern in CLAUDE.md): two new models, VtctRegistration (one per student, enforced in app code by find-then-upsert rather than a DB constraint, exactly like HomeworkSubmission\'s existing comment explains for the same deploy-gate reason) and VtctRegistrationDocument (kind: PHOTO_ID | PROOF_OF_ADDRESS | PRIOR_QUALIFICATION), plus one new relation field on AcademyStudent (vtctRegistrations). Gender reuses the schema\'s existing inclusive Gender enum instead of adding a new one.',
      'Confirmation / resubmission rule: picked the simpler of the two options the ticket offered. A registration cycles SUBMITTED -> CONFIRMED (admin has checked it against the documents) -> if the student edits it again, CHANGES_PENDING (the edit saves immediately, so nothing is lost, but it flags for admin re-review instead of silently overwriting the confirmed record) -> back to CONFIRMED once admin re-confirms. Implemented in lib/vtct-registration.ts (submitRegistration/confirmRegistration).',
      'Student form: components/academy/VtctRegistrationForm.tsx on a new /academy/vtct-registration portal page (added to the portal nav), matching the existing Academy UI kit (components/academy/ui.tsx -- Card/Pill/AButton, no new visual patterns). Conditional Yes/No reveal for the prior-VTCT question, per-document-kind upload groups, and the exact Student Declaration text from the ticket rendered verbatim with a checkbox that gates the submit button -- both client-side gating and a server-side re-check in submitRegistration (the client is never trusted alone for the mandatory-field/mandatory-document/declaration gate).',
      'Admin views: a VtctRegistrationPanel card on the existing student profile (app/admin/academy/students/[id]/page.tsx) showing status, every field and download links for every document, plus a Confirm action; and a review queue at /admin/academy/vtct-registrations (linked from the K Academy hub, same pattern as Portfolios/Homework) for staff to work through pending submissions without opening each student individually.',
  ...
  {
    title: 'Card-on-file no-show protection: admin-sent secure link silently failed to reach clients; no self-service way to add one',
    type: 'ERROR', urgency: 'P0', status: 'SHIPPED', assignee: 'claude',
    value: 10, effort: 5,
    detail: 'BLD-1797, client-reported: staff could not get clients a working "save a card, no payment taken" link for no-show protection via app/api/admin/bookings/request-card, tried by Email link / Text link / Both. Root causes found in the send path (app/api/admin/bookings/request-card/route.ts): (1) lib/sms.ts sendSms() deliberately returns { ok: true, dummy: true } when Twilio is not configured (a documented BLD-583 non-breaking fallback), but this route checked only `r.ok`, so an unconfigured/misconfigured Twilio account showed staff "Sent by sms ✓" while nothing was ever transmitted, with no error and no Sentry event -- a confirmed silent failure. (2) a client who has opted out of SMS (Client.smsReminders === false) fell straight through to a generic "check the client has an email/phone" message with no mention of the real reason. (3) a "Both" send that half-failed (e.g. email sent, SMS did not) reported unqualified success with no sign the other channel never went out. Separately, and reachable even when delivery itself worked: lib/client-auth.ts createAccountInvite() (used for every passwordless/migrated client -- the majority of phone/walk-in bookings this feature targets) minted its one-time activation token into the SAME resetTokenHash/resetTokenExp columns used by the client\'s own self-service "Forgot password" flow (requestPasswordReset() also calls createAccountInvite() for a passwordless account). Either a second admin resend (e.g. staff try "Email link" then "Text link" for the same client) or the client themselves clicking "Forgot password" while waiting silently overwrote and invalidated whichever link -- reset or invite -- had been issued first, so a client could receive a genuinely working email and still land on "This link may have expired" days later with nothing logged anywhere to explain why. Also found and fixed in the same area: lib/client-auth.ts requestPasswordReset() built its email link from `process.env.NEXT_PUBLIC_SITE_URL || \'\'` with no fallback to the canonical site.url that every other link-builder in the codebase uses -- if that env var were ever unset at runtime it would silently email a bare relative link. Part 2: no self-service place existed for a client to add/check/replace a card themselves outside a one-time emailed/texted link.',
    notes: [
      'Fix (Part 1, delivery bug): app/api/admin/bookings/request-card/route.ts now treats an SMS `dummy` result as NOT sent -- surfaces "SMS is not configured (Twilio credentials missing)" to staff and raises a Sentry warning (this flow is client-facing and P0, unlike the generic sms-test route which already handled `dummy` correctly and was the reference for this fix). An smsReminders-opted-out client now gets an explicit "this client has opted out of SMS" reason instead of the generic fallback message. A successful-but-partial send (e.g. "Both" with only email going through) now returns a `warnings` array alongside `ok: true`, which components/admin/RequestCardButton.tsx appends to the "Sent ✓" message instead of staying silent about the failed half.',
      'Fix (Part 1, token-clobbering): added Client.inviteTokenHash / Client.inviteTokenExp (additive, nullable -- no existing column touched, no @unique added) so the passwordless account-activation token has its own storage, separate from resetTokenHash/resetTokenExp. createAccountInvite() and activateAccount() (lib/client-auth.ts) now read/write the new pair; the daily cron (app/api/cron/daily/route.ts) sweeps expired invite tokens the same way it already sweeps expired reset tokens. A genuine password-reset request and a card-on-file invite can no longer invalidate each other. Also added the missing `|| site.url` fallback in requestPasswordReset()\'s link builder. NOT fixed here, flagged for a follow-up: a second admin resend of the invite (e.g. "Email link" then later "Text link" for the same still-unopened client) still supersedes the earlier still-valid invite, because each call mints one fresh token into the same single-slot column -- doing this correctly needs either a small dedicated invite-token table (supporting multiple concurrent outstanding invites) or a stateless signed-token scheme, both of which are auth-adjacent design decisions this P0 pass deliberately did not make unilaterally. See report for detail.',
      'Fix (Part 2, self-service section): added Client.stripeDefaultPaymentMethodId (additive, nullable) as the client-level "card on file" status, distinct from the existing per-booking Booking.stripePaymentMethodId. New app/api/account/card-intent/route.ts (session-authed) starts a Stripe SetupIntent for the client\'s own Stripe customer -- the exact same SetupIntent + Stripe Elements mechanism the admin-sent link already uses (components/booking/CardOnFileForm.tsx), no second payment-collection path invented. New app/api/account/card-saved/route.ts confirms it (ownership checked by matching the SetupIntent\'s customer to the signed-in client\'s stripeCustomerId, since the intent id itself is derivable from its own client secret the browser already held), sets it as the Stripe customer\'s default payment method, stamps stripeDefaultPaymentMethodId, and -- to make the protection actually take effect, since charging reads Booking.stripePaymentMethodId directly, not the customer default -- backfills it onto any of the client\'s own open upcoming bookings that do not already have a card (never overwrites one that does). New components/portal/CardOnFileSection.tsx (EN/UK via lib/i18n-portal.ts) renders the status ("Card on file: Visa •••• 4242, expires 04/27" or "No card on file") plus an Add/Update action, added as a section in app/account/profile/page.tsx alongside the existing Data & privacy section -- matching that page\'s established pattern rather than adding a new top-level portal nav item. app/api/booking/card-saved/route.ts (the existing admin-link save path) now also stamps stripeDefaultPaymentMethodId, so a card saved either way shows correctly in both places.',
      'Not touched: Stripe webhook logic, and no code path in this change ever handles a raw card number/CVV -- only Stripe SetupIntent ids and PaymentMethod ids, exactly as before.',
      'Verified: npx tsc --noEmit passes clean; npm run build passes clean (DB_SYNC_NONFATAL=true; sandbox cannot reach the production Postgres host). Not independently verified in this pass: an actual Twilio/Resend send in production (no live credentials in the sandbox), and a live click-through of the account/activate and account/card flows against a real database.',
    ],
  },
  {
  ...
  {
    title: 'Meta CAPI events had weak Event Match Quality; booking refunds never adjusted the Google Ads offline conversion',
    type: 'TASK', urgency: 'P2', status: 'SHIPPED', assignee: 'claude',
    value: 8, effort: 3,
    detail: 'Two independent ad-tracking accuracy gaps, plus one re-reported finding that was already fixed. BLD-1805: re-reported that components/shop/CheckoutForm.tsx, components/academy/EnrolmentCheckout.tsx and components/gift/GiftVoucherFlow.tsx fire no checkout-initiation pixel -- on inspection all three already fire begin_checkout/InitiateCheckout the moment their Stripe payment step is reached (BLD-1310, shipped earlier), matching components/booking/BookingFlow.tsx\'s own pattern; no code change needed, ticket is stale. PRJ-1200.4: lib/conversions.ts metaEvent() -- used by every server-side Lead/Schedule/Purchase CAPI event -- only ever sent a hashed email in user_data, omitting fbc/fbp (the visitor\'s own Meta Pixel cookies), client_ip_address and client_user_agent, which Meta weighs heavily for Event Match Quality; lib/attribution.ts also never captured fbclid alongside the gclid it already keeps. PRJ-1200.3: the refund path in lib/booking-actions.ts (refundBooking) called sendRefund(), which only ever sent a GA4 refund event -- it never told Google Ads the original offline conversion\'s value had changed, so Smart Bidding kept optimising toward the full pre-refund booking value indefinitely, even after a full refund.',
    notes: [
      'Fix (PRJ-1200.4): metaEvent() (lib/conversions.ts) now accepts optional fbc/fbp/clientIp/userAgent and adds them to user_data alongside the existing hashed email; sendLead(), sendSchedule() and metaPurchase()/sendPurchase() all take and forward the same four fields. Wired with real values at every call site that runs inside a live customer request (app/api/consult, app/api/finder-lead, app/api/account/signup, app/api/kiosk/results/[id]/claim, both app/api/booking/start sendSchedule calls, app/api/gift-vouchers/confirm) via a new lib/attribution.ts metaCookiesFromHeader() helper (reads the _fbc/_fbp cookies straight off the request, same regex-header style as the existing consentFromCookieHeader) plus lib/security/guard.ts\'s existing clientIp() and the request\'s user-agent header. Purchase-send sites that run from a Stripe-webhook backstop with no live customer request (lib/booking-actions.ts chargeBookingConfirmed, lib/shop.ts finalizeOrder, lib/academy-payments.ts) were left as hashed-email-only rather than attaching the wrong (webhook caller\'s) IP/UA -- not a regression, matches their existing behaviour. lib/attribution.ts Attribution gained fbclid, captured in attributionFromUrl() exactly like the existing gclid, and Booking.fbclid (additive, nullable) plus lib/marketing.ts bookingAttribution() carry it through to booking creation for later matching, mirroring gclid\'s own storage/reuse pattern.',
      'Fix (PRJ-1200.3): added uploadGoogleAdsConversionAdjustment() to lib/google-ads-conversions.ts, mirroring uploadGoogleAdsConversion()\'s auth/config/client-setup exactly, calling Google Ads customers:uploadConversionAdjustments with adjustmentType RESTATEMENT (adjustedValue = the booking\'s remaining net value) or RETRACTION (fully refunded, value 0), matched back to the original upload by orderId (the bookingId already set on it) rather than gclid+conversionDateTime. sendRefund() (lib/conversions.ts) now also takes gclid and adjustedValuePence, and calls it -- gated on marketingConsent, same purpose-gating as the original conversion upload -- whenever both are present. lib/booking-actions.ts refundBooking() now passes booking.gclid, booking.marketingConsent and adjustedValuePence = max(0, booking.chargedPence - totalRefunded) (the net value after this and any earlier refunds) into sendRefund().',
      'Verified: npx tsc --noEmit passes clean; npm run build passes clean (DB_SYNC_NONFATAL=true; sandbox cannot reach the production Postgres host).',
    ],
  },
  {
    ...
    notes: [
      'Fix (1): linkBookingToPackage\'s transaction now also zeroes the primary (non-addon) BookingItem\'s pricePence/discountPence and nets that same amount off Booking.pricePence, inside the same Serializable transaction as the occupancy check -- an add-on booked in the same slot is untouched (the package covers the primary treatment only, mirroring booking-creation-time pricing). No schema change.',
      'Fix (2): added five additive, nullable columns to Booking -- manualPaymentStatus (\'PAID\' | \'PARTIALLY_PAID\' | \'NOT_PAID\'), manualPaymentMethod, manualPaymentAmountPence, manualPaymentAt, manualPaymentBy -- plus a new gated server action setPackageManualPayment (app/admin/bookings/actions.ts, requires bookings.charge, same permission as every other "money has moved" action) and a new PackagePaymentControl dialog wired into the Packages card on the client profile. clientPackages() now folds manualPaymentStatus === \'PAID\' into the existing paid boolean (alongside chargedAt/prepaidAt) -- \'PARTIALLY_PAID\' deliberately does NOT flip paid, mirroring the existing bar that already requires a package be paid in full before the client-facing booking flow will spend a session from it. The client-profile card shows a distinct "Partially paid" badge state and a "Recorded by staff -- method, amount, date" line when an override is set.',
      'Pre-merge review fixes (money path, so listed in full). (a) The zeroing now only runs for an appointment that actually takes a session (`occupies`). On a CANCELLED-late or NO_SHOW booking pricePence is not a price -- it IS the unwaived fee lib/outstanding.ts derives the client debt from (pricePence > 0) -- so zeroing it wrote the debt off silently while leaving the session unspent (a link consumes nothing; staff still have to mark the session used, which is how BLD-1347 takes the fee out of the course). (b) The already-paid guard checked chargedAt only; prepaidAt joined it, the same gap BLD-1119/BLD-1200 closed in the session route and outstandingBalance -- a BNPL settlement never touches chargedAt, so a paid visit could have been handed back AND spent a prepaid session. (c) Linking is refused when a gift voucher is applied: reserveVoucher has already debited the card, and zeroing the price stranded that balance against a GBP 0 visit. The error names the Remove voucher control. (d) Loyalty points redeemed against the appointment are returned via refundBookingPoints when the price is zeroed, the same rule the cancellation/refund paths follow -- otherwise the client paid points for a visit the course covers. (e) The zeroing was irreversible: unlinkBookingFromPackage detached the booking but left it at GBP 0 for ever, and nothing recorded what had been destroyed. The link now writes the pre-link amounts to its own audit entry (AuditEvent.meta.packagePriceZeroed) and unlink restores them exactly, guarded so it never overwrites a later overrideBookingPrice or a payment. (f) setPackageManualPayment now validates `status` at runtime -- a server action is a public POST endpoint and the union type is erased, so any string could have landed in the column every reader compares to PAID/PARTIALLY_PAID.',
      'Pre-merge review fix (UI): PackagePaymentControl\'s dialog is now rendered through createPortal to document.body. <Dialog> renders inline, and the control sits inside the Packages card\'s <Link>, so the whole modal -- backdrop, select, inputs, Cancel and Save -- was a DOM descendant of a real <a href>. stopPropagation only stops Next\'s Link click HANDLER; the anchor\'s default navigation is cancelled by preventDefault and nothing else, so every click inside the dialog triggered a full-page navigation to the booking -- clicking Save started the server action and then immediately navigated off it. The portal takes the modal out of the anchor entirely (and out of invalid interactive-inside-<a> nesting); React events still bubble through the React tree, so the wrapper\'s stopPropagation is still needed and still works.',
      'Second-pass review fixes. (g) priorPackageZeroing() read only the 25 most recent SESSION_EDITED audit rows for the booking, and SESSION_EDITED is a busy action there (add-on added/removed, session notes, price override, mark/unmark package session used, plus the system row at creation). Once enough landed after the link, the link\'s own entry fell out of the window and unlink silently restored nothing -- the appointment stayed at GBP 0 for ever. Window widened to 200. (h) The migration used plain ADD COLUMN; every recent migration in this repo uses ADD COLUMN IF NOT EXISTS because deploys still run prisma db push, so these columns will already exist the first time migrate deploy runs and the plain form would fail the deploy outright. Rewritten in the house shape.',
      'Residual, not changed here (flagged for a follow-up item): the undo depends on AuditEvent.meta as its only record of what the link zeroed, and that write is best-effort (logAudit swallows its own failures). If the audit row never lands, the money is off the appointment with nothing able to put it back. Storing the zeroed amount + item id on the Booking (two more additive nullable columns) would make the undo deterministic; that is a design change rather than a pre-merge fix.',
      'No schema-destructive change -- five new nullable columns, no @unique added. Verified: npx tsc --noEmit passes clean; npm run build passes clean (DB_SYNC_NONFATAL=true; sandbox cannot reach the production Postgres host).',
    ],
  },
  {
    ...
    notes: [
      'Fix: the headline status is now derived from the already-computed per-variant effective statuses instead of a bare Service-status scan -- NORMAL (bookable) if ANY variant under the treatmentSlug is NORMAL, else CONSULTATION if any is CONSULTATION, else COMING_SOON if any is COMING_SOON, else UNAVAILABLE if any is UNAVAILABLE. This means a treatment reads as bookable the moment ANY of its Service rows is, matching what an admin who fixed one of them would expect, while a treatment where every Service row is still non-bookable keeps showing Coming soon/Unavailable as before.',
      'Pre-merge review fix: the first cut folded CONSULTATION into NORMAL and defaulted to NORMAL when a treatment had no active variants at all. Both were wrong. CONSULTATION is bookable but drives distinct behaviour a bare NORMAL loses -- the "On consultation" badge on TreatmentCard, the "Free consultation" secondary CTA on the treatment page (BookingButtons `consult`), and the GBP 0 card-on-file hold in /api/booking/create -- so a consultation-only treatment would have lost all three. And a Service row created but not yet given variants (status COMING_SOON, no variants to scan) would have been published as bookable with no price; that case now keeps the pre-BLD-1826 Service-level reading.',
  ...
  },
  {
    ...
    notes: [
      "Fix: gave the plain-text composer the same shape as the block-based one. sendCampaign() now creates the campaign with status: 'SENDING' up front (visible in the campaigns History list as \"sending...\" instead of silently reading as a draft), and delivery moves to a new deliverPlainCampaign() in lib/email-campaigns.ts -- cursor-paginated recipients (bounded memory on any list size), bounded concurrency (8) over the send (the shared Resend rate gate in lib/email.ts still serialises the actual network calls, so this can't trip the 4.5/sec cap), and an EmailEvent written per recipient immediately so re-running for the same campaignId only mails whoever doesn't have one yet. campaign.recipients is also persisted after every batch, not just at the end, so a killed invocation still shows real progress rather than 0.",
      "Resume path: added resumeStuckPlainCampaigns() to lib/email-campaigns.ts, called from dispatchDueCampaigns() -- already invoked every 15 minutes by the existing app/api/cron/dispatch cron route, so no new route or schedule entry was needed. It picks up any campaign still SENDING after the same 30-minute stale cutoff the block-based sweep uses and finishes it. Discount-code settings (type/value/expiry) are now persisted on the Campaign row (new nullable format/discountType/discountValue/discountExpiresAt columns, additive -- no @unique, no destructive change) so a resumed send mints the remaining recipients' codes on the same terms as the original attempt, not recomputed from a fresh 'days from now'.",
      "The two sweeps stay mutually exclusive without branching on message content: the existing block-based stuck-SENDING query in dispatchDueCampaigns() now filters audienceType: { not: null } (always set by the rich composer, never by the plain one), and the new sweep filters format: 'text' (only ever set by the plain composer) -- so a plain-text campaign can never be picked up by the JSON.parse(body) path and mailed as an empty-content blast to 'all', and vice versa.",
      "Also added export const maxDuration = 60 to app/admin/campaigns/page.tsx (not actions.ts -- a 'use server' file may only export async functions, and Vercel applies a page/route segment's maxDuration to Server Actions invoked from it), matching the 60s convention used on every other long-running route in this app -- the batching + resume is what actually makes a large send safe, this just keeps a single invocation's ceiling predictable and explicit rather than implicit.",
  ...
  {
    title: 'Restore Klarna & Clearpay across the site, matched to where BNPL is actually real', type: 'TASK', urgency: 'P2', status: 'IN_REVIEW', assignee: 'claude',
    value: 6, effort: 3,
    detail: "BLD-1827 asked to make Klarna/Clearpay available and visible on treatment, pricing, booking, checkout, Academy course and FAQ surfaces. The premise of the ticket -- that the treatment-booking checkout needs re-wiring from payment_method_types: ['card'] to automatic_payment_methods -- turned out to be wrong on investigation, so this shipped as a copy-and-scope change with NO Stripe configuration change at all. Two findings drove that. (1) Every one of the four card-only sites named in the ticket is a SetupIntent, not a PaymentIntent: app/api/booking/create/route.ts:279, app/api/booking/start/route.ts:457, app/api/account/card-intent/route.ts:34 and app/(marketing)/booking/card/page.tsx:37. They save a card off-session for no-show / late-cancellation / post-treatment charging; no money moves. Klarna and Clearpay cannot be saved off-session for later reuse -- the codebase already records this at app/api/admin/bookings/bnpl-link/route.ts:11 (\"BNPL methods can't be saved off-session\") -- so switching any of them to automatic_payment_methods would either be rejected by Stripe or produce a broken option. All four correctly stay card-only. (2) Every real self-service charge in the codebase ALREADY uses automatic_payment_methods AND already renders a Stripe PaymentElement, so BNPL is already selectable end-to-end with no change needed: Academy enrolment (lib/academy-payments.ts:261-267 + components/academy/EnrolmentCheckout.tsx), shop checkout (app/api/shop/checkout/route.ts:141-142 + components/shop/CheckoutForm.tsx) and gift vouchers (lib/gift-vouchers.ts:150-151 + components/gift/GiftVoucherFlow.tsx). There is no CardElement anywhere in the repo -- every Stripe surface uses PaymentElement -- so the front-end gap the ticket anticipated does not exist either. The real, accurate position is that online treatment booking is not a checkout at all and therefore can never carry a BNPL promise, while BNPL on a treatment is genuinely available through a staff-sent hosted Stripe Checkout link (app/api/admin/bookings/bnpl-link/route.ts for a course, and the 'paylink' case in app/api/admin/bookings/session/route.ts:200 for a booking balance), plus in-clinic POS (app/api/admin/pos/route.ts:173). This entry exists because the claim-vs-reality gap BLD-1006 closed was reopened once already by BLD-1829, and the fix this time is to pin every new claim to a specific verified code path rather than to restore generic copy.",
    notes: [
      "Genuinely BNPL-capable end-to-end, verified in code, and now claimed in copy: (a) KClinics Academy enrolment -- self-service, client picks Klarna/Clearpay in the PaymentElement; (b) online shop orders -- self-service; (c) gift cards and gift packages -- self-service; (d) treatment courses, packages and booking balances -- real but STAFF-INITIATED only, via a hosted Checkout payment link a staff member sends; (e) in-clinic POS card sales -- staff-initiated hosted Checkout. No code change was needed for any of these; they were already wired.",
      "Deliberately NOT claimed anywhere, and deliberately left unfixed: the self-service online booking flow. It takes no payment whatsoever -- it creates a SetupIntent to save a card, and the treatment is charged later off-session or paid in clinic. Adding client-selectable BNPL there is not a config flag; it would mean building an online treatment payment/deposit flow that does not exist today (new charge-at-booking path, refund and reschedule handling, webhook cases, and a decision about taking money before a clinician has assessed suitability). That is a separate feature, not part of this pass, and is the one part of BLD-1827 that is NOT delivered. Flagging it explicitly rather than quietly implying it is covered.",
      "Copy changes, all using the eligibility-qualified phrasing already on /finance (\"subject to the provider's own terms, eligibility checks and approval decision\") and never a stronger claim: components/ui/PaymentMarks.tsx -- Klarna and Clearpay restored to the sitewide footer strip and to its aria-label, as palette-tinted word marks matching the existing Visa/AMEX/GPay treatment. Deliberately NOT the colour /treatments/Klarna.svg asset: that strip's documented design contract is that no mark uses its official brand colours, and no Clearpay artwork exists in the repo or was fabricated. lib/faqs.ts -- rewrote 'Can I pay in instalments?' to name Klarna/Clearpay with their exact scope, and added a new 'Do you accept Klarna or Clearpay?' item; both state plainly that booking online never takes a payment. components/treatment/TreatmentTemplate.tsx -- a line under the price (hidden for enquiry-only treatments) saying BNPL is available on eligible courses and packages via a payment link from the team, which is exactly what the code does. app/(marketing)/pricing/page.tsx -- a 'Spread the cost' block above the price-list footnote, splitting Academy (at checkout) from treatments (payment link). app/(marketing)/academy/[slug]/page.tsx -- the course-fee note now names Klarna/Clearpay at checkout, which is true and already matched components/academy/EnrolmentCheckout.tsx:101.",
      "Untouched on purpose, per the ticket's scope limits: all Stripe webhook handling, refund logic, lib/academy-payments.ts, app/api/admin/bookings/bnpl-link/route.ts, components/academy/ApplyForm.tsx and app/(marketing)/finance/page.tsx. The /finance copy re-added by BLD-1829 was re-checked against the code during this pass and is accurate as written -- it claims BNPL on 'eligible treatments, packages and KClinics Academy courses' without promising it at online booking, and every one of those is reachable through a real path -- so it was left exactly as-is rather than edited.",
      "Net effect on Stripe: zero. No payment_method_types value was changed, no automatic_payment_methods was added or removed, no intent type was changed. This PR is display and copy only, scoped so that every Klarna/Clearpay claim on the site now points at a code path that can actually honour it. (BLD-1827)",
    ],
  },
];
