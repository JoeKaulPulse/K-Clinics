...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  {
    title: 'Add Sentry error tracking (BLD-348)', type: 'TASK', urgency: 'P2', status: 'SHIPPED', assignee: 'claude',
    value: 3, effort: 1,
    detail: 'Integrated @sentry/nextjs. sentry.client.config.ts, sentry.server.config.ts, sentry.edge.config.ts initialise the SDK only when SENTRY_DSN / NEXT_PUBLIC_SENTRY_DSN is set so the build is a no-op without credentials. next.config.mjs wrapped with withSentryConfig (tunnelRoute: /api/sentry so client events route through the app -- no CSP changes needed; source-map upload disabled when SENTRY_AUTH_TOKEN is absent). tracesSampleRate 0.1; Replay integration captures on errors only (replaysOnErrorSampleRate 1.0) to keep quota low.',
    notes: ['Env vars to set in Vercel: SENTRY_DSN (server/edge), NEXT_PUBLIC_SENTRY_DSN (client), SENTRY_ORG, SENTRY_PROJECT, SENTRY_AUTH_TOKEN (for source-map upload in CI). Without these the app runs identically with no SDK overhead.'],
  },
  {
    title: 'Order number race: replace count()-based KC#### with atomic Setting counter (BLD-332)', type: 'ERROR', urgency: 'P2', status: 'SHIPPED', assignee: 'claude',
    value: 2, effort: 1,
    detail: 'nextOrderNumber() in lib/shop.ts replaced count()-based generation with a Postgres UPSERT on the Setting table (key _order_seq). INSERT seeds from the current max KC#### order number; ON CONFLICT atomically increments. Two concurrent checkouts race safely -- Postgres serialises the UPSERT so each caller gets a distinct value. No schema change required (reuses the existing Setting PK on key).',
    notes: ['lib/shop.ts nextOrderNumber(). First call seeds from MAX(KC-number) to avoid colliding with any orders already generated by the old path. Subsequent calls increment by 1.'],
  },
  {
    title: 'Refund CAS: guard side-effects in refundBooking() against webhook race (BLD-355)', type: 'ERROR', urgency: 'P2', status: 'SHIPPED', assignee: 'claude',
    value: 2, effort: 1,
    detail: 'lib/booking-actions.ts refundBooking(): the plain db.booking.update is replaced by a CAS db.booking.updateMany that checks refundedPence has not changed since we read the booking. If count=0 (a concurrent webhook echo or duplicate call already applied the update) we return early before running loyalty-reversal and Xero side-effects. Mirrors the same CAS pattern already used in the charge.refunded webhook handler. The existing Stripe idempotencyKey on the refund create remains unchanged.',
    notes: ['lib/booking-actions.ts ~line 191. Webhook handler (app/api/stripe/webhook/route.ts) already had CAS + the originatedInApp guard. In-app path now symmetric.'],
  },
  {
    title: 'GDPR Art.15 SAR export completeness + Art.17 order erasure (BLD-315)', type: 'TASK', urgency: 'P2', status: 'SHIPPED', assignee: 'claude',
    value: 2, effort: 2,
    detail: 'app/api/account/export/route.ts: SAR export now covers every category the Art.17 erasure path touches -- added aiAnalysis (metadata), signedConsent (metadata, no body/sig), beforePhoto (metadata, no image), review, npsResponse, followUp, referral, chatConversation (messages included), waitlist, callRecord (safe metadata only), orders (by email for legacy rows without clientId). app/admin/actions.ts eraseClientData: added Order.updateMany to the $transaction -- strips email/name/phone/shipAddress from all orders matched by clientId or email; retains financial amounts and status for HMRC.',
    notes: ['No new models or schema changes. Order erasure pre-fetches the client email before the $transaction to support the OR(clientId, email) match. Encrypted payloads (health assessment answers, consent body, before-photo image) remain excluded from the SAR -- they are shared on a verified identity request per the privacy notice.'],
  },
];
