...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  },
  {
    ...
    notes: [
      ...
      'BLD-1463: removed GeistMono from app/layout.tsx (root <html> class). It is applied instead inside app/admin/layout.tsx and app/account/layout.tsx (each on their own wrapper div, since only the root layout can set classes on <html>/<body>), so the font is preloaded only on the routes that actually render it. A small number of secondary marketing pages that also reference --font-mono for code/reference display (gift voucher preview, checkout gift-card-code field, academy certificate/verify reference, the isDemo-only demo card fields) now inherit the surrounding body font there instead of true monospace -- a deliberate, low-visual-impact trade-off; --font-mono in app/globals.css already chains to ui-monospace/monospace so nothing renders broken.',
      'Verified: npx tsc --noEmit and npm run build pass clean (npm run build validated with the sandbox DB-connection vars unset, since raw Postgres TCP is not reachable from this network policy -- prebuild db-sync is a no-op without them).',
    ],
  },
];
