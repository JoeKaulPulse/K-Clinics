...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  },
  {
    ...
    notes: ["Fix: added enforceRateLimit(req, 'chat-poll', 60, 60) to the GET handler -- a higher allowance than POST's 30/60 since the widget polls every ~4s while open. Reviewed independently (max-effort pass): signature verified against lib/security/guard.ts, degrades softly on 429 (client already tolerates {ok:false, messages:[]} and retries). tsc and build both pass. (PRJ-1069.3)",
      ...
      "Fix: app/api/consent/[token]/route.ts GET now calls enforceRateLimit(req, 'consent-read', 20, 600), matching the sibling /api/consent/sign, and logs a new CONSENT_READ audit event (additive AuditAction enum value) whenever the endpoint actually returns a template's content + client name for a token. Reviewed independently: the enum addition passes the prisma db push deploy gate and doesn't break the admin activity feed's icon lookup (unknown actions render a fallback icon). (PRJ-1069.7)",
      ...
      'Shipped together as PR #1730 on claude/security-reliability-batch (commit 39b66c3b), reviewed at max effort with no defects found. Residual, non-blocking risk noted by review: if Upstash Redis is not linked in production, the shared enforceRateLimit falls back to a Postgres SecurityEvent insert-per-request -- worth confirming the Redis check is green on /admin/api-health post-deploy. Rollback: git revert 39b66c3b.'],
  },
  {
