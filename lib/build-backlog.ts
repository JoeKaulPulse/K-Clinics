...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  {
    title: 'Allow GBP 0 as an appointment price (BLD-1869)',
    type: 'ERROR', urgency: 'P2', status: 'IN_REVIEW', assignee: 'claude',
    value: 5, effort: 2,
    detail: "BLD-1869: Booking.pricePence 0 was overloaded as the sentinel for 'on consultation -- amount set at charge time' (the comment on prisma/schema.prisma's pricePence field said exactly that), so there was no way to tell a never-priced appointment from one staff had deliberately priced at GBP 0. overrideBookingPrice (app/admin/bookings/clinical-actions.ts, BLD-1149) has always ACCEPTED 0 -- it only rejects a negative amount -- so staff could already save a free appointment; every display then silently relabelled it 'On consultation', which is wrong in front of a client who has been told the visit is free. This was purely a display bug: no pricing, payment or catalogue logic needed changing.",
    notes: [
      "Schema: one new additive, nullable Booking.priceOverriddenAt DateTime (no default, no @unique -- CLAUDE.md bans adding a unique constraint to an existing table), plus migration 20260922120000_booking_price_overridden_at using ADD COLUMN IF NOT EXISTS, matching the convention every recent migration follows since deploys run prisma db push without --accept-data-loss. Same shape as Client.patchTestRecordedAt (BLD-1846).",
      "overrideBookingPrice now writes priceOverriddenAt: new Date() alongside pricePence in the same db.booking.update inside the existing $transaction. Every guard is untouched: the bookings.charge permission, the non-empty trimmed reason (max 200 chars, still required client-side in components/admin/PriceOverride.tsx too), sessionIsAdmin for a post-payment correction, and the prepaidAt / CANCELLED / NO_SHOW refusals. The primary BookingItem sync and the audit entry are unchanged. Nothing writes to the treatment catalogue -- the flag is per-appointment, so the treatment's standard price and its own 'On consultation' behaviour are unaffected.",
      "Display rule applied at every booking-level price site: if priceOverriddenAt is set, always render the literal money value of pricePence (including GBP 0); otherwise the existing pricePence > 0 ? money(...) : 'On consultation' behaviour is unchanged. Sites: app/admin/bookings/[id]/page.tsx (headline price, the Treatments & billing base line, and the per-session figure on a course), components/admin/session/SessionRunner.tsx checkout (new priceOverridden boolean prop, passed from app/admin/bookings/[id]/session/page.tsx), app/admin/bookings/page.tsx list row (it printed an em-dash for GBP 0), lib/booking-notify.ts (money() gained a `priced` flag, threaded to the line items and both email templates), lib/email.ts tmplBookingNotify (the staff 'New booking' email) and tmplBookingConfirmation (the client's confirmation, which said 'Assessed at your visit'), components/live/LiveCompanion.tsx via a new ClientLiveView.pricing.priceOverridden in lib/appointment-session-server.ts (the client's in-clinic live screen, whose price card also no longer hides itself on a GBP 0 staff-set price), and app/(marketing)/booking/manage/ManageClient.tsx (also 'Assessed at your visit').",
      "Four of those sites were not in the original ticket's list: they had the same bug with different wording (an em-dash, 'Assessed at your visit', or a money() helper that folded the check inside itself), so a grep for the exact pricePence > 0 ? money(...) : 'On consultation' pattern missed them. Fixing only the ones that used the literal string would have left a staff-set GBP 0 still reading as 'On consultation' on the client's own phone and confirmation email.",
      "Deliberately NOT changed: lib/services.ts fromLabel/formatPence/statusLabel and every marketing 'from GBP X' / 'On consultation' badge -- those describe a TREATMENT's own catalogue price, where 0/null correctly means priced at consultation. components/admin/NewBookingButton.tsx priceLabel is catalogue variant and course pricing inside the create-booking modal, before any Booking row exists, so there is nothing per-appointment to honour there. app/account/appointments/page.tsx gates the RedeemPoints control on pricePence > 0, which is behaviour not display and is correct (points cannot be redeemed against a GBP 0 visit); likewise the pricePence > 0 guards in lib/booking-actions.ts (late-cancellation fee, charge path), lib/outstanding.ts and lib/client-loyalty.ts are money logic and were left alone.",
      "Adjacent hole found but NOT fixed here, flagged for its own ticket: unlinkBookingFromPackage (app/admin/bookings/actions.ts) restores the price that linkBookingToPackage zeroed, and guards that restore with item.pricePence !== 0 meaning 'staff have re-priced it since, leave it alone'. A staff override TO GBP 0 leaves pricePence at 0, so the guard does not see it and the unlink overwrites a deliberate GBP 0 with the pre-link amount. That is pre-existing (the guard behaved the same before this change) and priceOverriddenAt alone cannot fix it: an override made BEFORE the link also sets the timestamp, so the check would have to compare priceOverriddenAt against the link audit entry's createdAt. Left alone rather than changing a money path outside this ticket.",
      "Verified: npx tsc --noEmit passes clean; DB_SYNC_NONFATAL=true npm run build passes clean (this sandbox cannot reach the production Postgres host over raw Postgres, so the prebuild db-sync step is expected to fail here -- DB_SYNC_NONFATAL is the existing opt-in for exactly that, used for local verification only, not set anywhere in committed config).",
    ],
  },
];
