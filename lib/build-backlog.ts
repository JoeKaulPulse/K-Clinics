...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  {
    title: 'Gift-card strap-line, TreatmentCard hover contrast, star-rating aria-label, broken npm run lint (BLD-1239, BLD-1246, BLD-1262, BLD-1214)', type: 'TASK', urgency: 'P1', status: 'SHIPPED', assignee: 'claude',
    value: 5, effort: 2,
    detail: 'Four small, independent fixes verified against current code before starting: (1) BLD-1239, GiftCardPreview.tsx rendered "Aesthetics . Laser . London" directly under the KMark logo -- a strap-line under the logo is explicitly banned by docs/BRAND_GUIDELINES.md; (2) BLD-1246, TreatmentCard.tsx set the 24px hover title to --color-gold, 2.755:1 on porcelain, below the WCAG AA 4.5:1 text minimum; (3) BLD-1262, the admin reviews Stars component rendered raw star/circle glyphs with no aria-label, so screen readers read either nothing meaningful or the literal glyph characters; (4) BLD-1214, package.json\'s lint script was still "next lint", a command Next.js 16 removed outright, so every session since the Next 16 upgrade got an immediate "Invalid project directory" crash instead of a lint run -- and no eslint.config.* file existed anywhere in the repo to invoke ESLint directly against either.',
    notes: [
      'Fix (BLD-1239): moved the strap-line out of the logo lock-up entirely. It now sits in the card footer next to the voucher code (KC-GV-....-....), stacked underneath it in a smaller uppercase line, so the KMark renders alone with nothing under it.',
      'Fix (BLD-1246): swapped group-hover:text-[var(--color-gold)] to group-hover:text-[var(--color-gold-deep)] on the title, matching the AA-safe token the same file already uses for the Offer badge and the Discover link.',
      'Fix (BLD-1262): wrapped the glyph run in an inner aria-hidden="true" span and moved the label to the outer span as aria-label -- "${n} out of 5 stars" when n is set, "no rating" when n is null/0 (the component already rendered a plain em-dash for that case).',
      'Fix (BLD-1214): confirmed by direct inspection (git ls-files, a filesystem scan of the repo root, and running npx eslint . cold) that no eslint.config.mjs/js/cjs or legacy .eslintrc.* existed at all -- CLAUDE.md and this task both assumed one was already in place, but the repo had never been migrated off next lint\'s implicit config resolution before that command was removed in Next 16, so there was nothing for a direct eslint invocation to find. Added eslint.config.mjs at the repo root using eslint-config-next\'s own native flat-config exports (eslint-config-next/core-web-vitals + eslint-config-next/typescript -- no @eslint/eslintrc FlatCompat shim needed, since eslint-config-next 16.2.12 already ships flat arrays directly). Changed package.json\'s lint script from "next lint" to "eslint .". eslint itself was only present as an indirect peer dependency of eslint-config-next (hoisted into node_modules/.bin by luck of the install order); added it as an explicit devDependency ("eslint": "^9.39.4", matching the version already resolved in the lockfile) so the lint command does not depend on hoisting behaviour holding up across installs.',
      'Verified: npx tsc --noEmit clean; npm run build (DB-less, DATABASE_URL/POSTGRES_URL* unset so prebuild\'s db-sync step skips rather than trying to reach the live Neon DB) completes 353/353 static pages with exit 0; npm run lint now runs to completion and reports real findings (289 problems: 188 errors, 101 warnings, all pre-existing and out of scope for this task) instead of the old "Invalid project directory" crash.',
    ],
  },
];
