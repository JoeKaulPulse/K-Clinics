...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  },
  {
    ...
    notes: [
      'Fix (BLD-1303): app/api/admin/staff/route.ts\'s create branch now requires actor.role === \'OWNER\' to assign any role other than the actor\'s own role -- mirroring the update path\'s existing gate exactly, rather than only special-casing role === \'OWNER\'. A non-owner can still create a peer at their own role (no escalation there); anything else 403s with "Only an owner can assign a role other than your own." Create and update are now consistent: both require OWNER for any actual role change, and both leave the permGrant/permRevoke clamp (clampGrant, ~L35) as the separate, already-correct guard against permission-level escalation.',
      ...
      'Deliberately left blocked rather than special-cased further: app/api/admin/profile/route.ts\'s updateProfile/changePassword ops (the ProfileEditor form rendered on the same confinement page) and the OWNER-only passkey-registration endpoints also rendered there. The finding scopes the carve-out to "the 2FA-setup and logout endpoints"; these are neither, so a pending session gets a 401 from them until enrolment completes, at the cost of those two form actions not working on the confinement page until then -- a narrower, more defensible allow-list than guessing at every UI affordance the page happens to render.',
