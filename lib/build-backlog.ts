...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  {
    title: 'Stuck-SENDING email campaigns, no timeout on web-push sends (PRJ-1043.5, BLD-1209)', type: 'ERROR', urgency: 'P1', status: 'IN_REVIEW', assignee: 'claude',
    value: 5, effort: 2,
    detail: "PRJ-1043.5: sendCampaignById() (lib/email-campaigns.ts) and the immediate-send route (app/api/admin/marketing/email/send/route.ts) both atomically claim SENDING via db.campaign.update(...) then call deliverCampaign() with no surrounding try/catch and no timeout -- a thrown error, or the serverless function getting killed at maxDuration=60 mid-loop, left the row stuck in SENDING forever. Nothing swept it (the cron dispatcher only queries SCHEDULED/AB_TESTING) and the delete route explicitly refused to remove a SENDING campaign, so a stuck row could not even be cleared manually. BLD-1209: lib/push.ts's sendPush() called webpush.sendNotification() inside Promise.all with no AbortSignal/timeout, unlike lib/email.ts's 10s Promise.race pattern -- one hung push endpoint stalled every send for a user with multiple devices.",
    notes: [
      'Fix (PRJ-1043.5): wrapped the deliverCampaign() call in both sendCampaignById() and the immediate-send route in try/catch. On catch, the campaign is put back to SCHEDULED with scheduledAt set to now (Campaign.status is a plain String column, not the CampaignStatus enum -- that enum belongs to the separate MarketingCampaign model -- so no schema change was needed) and the error is logged; the route also returns a 500 with a "requeued for retry" message instead of silently losing the failure. Added recoverStuckSendingCampaigns() to lib/email-campaigns.ts and wired it into app/api/cron/dispatch/route.ts using the file\'s existing try/catch-and-push-to-failures sweep pattern (same shape as releaseAbandonedPendingBookings): finds any campaign still in SENDING with updatedAt older than 12 minutes (comfortably past the 60s maxDuration on both send paths) and resets it to SCHEDULED, due now, so the next cron tick retries delivery. This is a general safety net keyed on status + updatedAt, so it also recovers a SENDING row left by startAbTest()/decideAbTest() or an older deploy without the try/catch, not just the two call sites patched directly.',
      "Fix (BLD-1209): applied lib/email.ts's exact Promise.race-against-a-10s-timeout pattern around each webpush.sendNotification() call in lib/push.ts, inside the existing per-subscription try/catch. A timed-out send now throws (caught by the same catch that already handles 404/410 subscription pruning) instead of leaving that Promise.all entry hanging, so one unresponsive endpoint can no longer stall notifications to a user's other devices.",
      'Verified: npx tsc --noEmit and npm run build (DB-less production build) both pass clean.',
    ],
  },
];
