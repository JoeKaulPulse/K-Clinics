...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  {
    title: 'A failed late-cancellation/no-show fee was never visible past a one-time toast', type: 'TASK', urgency: 'P1', status: 'IN_REVIEW', assignee: 'claude',
    value: 7, effort: 3,
    detail: 'When a late-cancellation or no-show fee charge was declined, cancelBooking() and recordChargeFailure() (lib/booking-actions.ts) only wrote a free-text Interaction note and a PAYMENT_FAILED audit event, plus a one-off email to the client. Nothing durable was saved: components/admin/BookingActions.tsx showed feeFailed only as a toast that disappeared on reload, staff had no persistent signal on the client profile or booking detail, and the client had no way to see it was still owed when they logged into their account or tried to book again -- so a declined fee routinely went uncollected and the client could rebook without ever settling it.',
    notes: ['Fix: added Client.outstandingPaymentPence (Int, default 0 -- additive, no destructive schema change) to prisma/schema.prisma, tracking the actual owed amount rather than a boolean. cancelBooking() increments it by the failed fee amount on a synchronous decline; recordChargeFailure() (the async Stripe-webhook decline path) does the same, gated on the PaymentIntent\'s own metadata.late flag (set by chargeBooking()) so an ordinary treatment-charge failure is not mistaken for a late fee. finalizeBookingCharge() clears it to zero when a late fee that had required card authentication later succeeds via the webhook/SCA-recovery path. chargeBookingAction() (app/admin/bookings/actions.ts, the existing manual "Charge card on file" action -- no new charge infra was built) also clears it to zero on any successful charge, since staff collect an outstanding balance by adding it to the client\'s next payment rather than re-billing the original (by then cancelled) booking. Full settle-to-zero on every success path for this first pass; a future pass could make it proportional. Warning banners (reusing the existing medicalFlag red-banner pattern) now render on: the admin client profile (app/admin/clients/[id]/page.tsx), the admin booking detail page whenever it is opened (app/admin/bookings/[id]/page.tsx, b.client.outstandingPaymentPence), and the client account dashboard on every login (app/account/page.tsx). app/api/booking/start/route.ts and app/api/booking/create/route.ts (the client-facing self-service booking endpoints only) now return 403 with an actionable message if outstandingPaymentPence > 0, soft-blocking a new booking until the balance is settled -- mirroring the existing 18+ age-gate 403 pattern. The admin booking-creation path (app/admin/bookings/create-action.ts) is untouched and unaffected: staff can still book an appointment for a client with an outstanding balance, e.g. specifically to help them settle it. tsc and build both pass. (BLD-1066)',
      'Pre-merge review, two defects found and fixed on this branch. (1) Double counting: a synchronous decline in cancelBooking() incremented the balance, and Stripe then delivered payment_intent.payment_failed for that SAME PaymentIntent, which ran recordChargeFailure() and incremented it again -- and that event can arrive more than once (a retry on the still-live PI, or a webhook redelivery), so the balance grew to a multiple of the real fee and soft-blocked the client for a debt they did not owe. Both paths now go through one accrueOutstandingLateFee() helper in lib/booking-actions.ts which claims the accrual atomically on a new additive Booking.lateFeeOwedAt DateTime? column (updateMany where lateFeeOwedAt: null), so a booking\'s failed fee is counted exactly once however many times it is reported. (2) Information disclosure: app/api/booking/create is the PUBLIC unauthenticated guest booking route, and the 403 echoed the exact outstanding amount back to whoever typed the email address, and was placed after the client upsert so a rejected attempt still rewrote that client\'s name/phone/dob. The check now runs before the upsert as a findUnique on the email and returns a generic "call us" message with no amount; app/api/booking/start keeps the itemised message because the caller there is authenticated as that client. (BLD-1066)',
      'Reviewed and NOT changed, flagged for a human decision: settlement is settle-to-zero, so any successful charge on any completed booking for that client (chargeBookingAction) or any successful late fee (finalizeBookingCharge with late) wipes the whole balance, including an older unrelated one -- staff must remember to add the owed fee to the amount they take or the money is written off. And there is no manual "clear balance" control: the balance can only be cleared by a successful CARD charge, so a client who settles in cash, or who has no card on file, stays soft-blocked from online booking indefinitely. (BLD-1066)'],
  },
];
