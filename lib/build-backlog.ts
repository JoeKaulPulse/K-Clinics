...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  {
    title: 'Rotating promotional hero banner on the homepage (BLD-1197)', type: 'TASK', urgency: 'P1', status: 'SHIPPED', assignee: 'claude',
    value: 6, effort: 3,
    detail: 'Owner-reported (info@kclinics.co.uk): the homepage hero only ever showed one static banner. Requested a rotating carousel so the clinic can feature different promotions and treatments -- slide 1 the existing branding, slide 2 a Signature Facial offer (150 pounds instead of 210), with future slides (Laser Hair Removal, HydraFacial Platinum, HIFU, seasonal offers, Black Friday) added later.',
    notes: [
      'Built as three files. components/home/hero-slides.tsx -- an ordered HERO_SLIDES array (BrandHeroSlide | PromoHeroSlide) so a third slide is just another object appended to the array; slide 1 is a bare { kind: "brand" } marker (its content depends on the live rating aggregate, so it is rendered from that data rather than carried as static copy), slide 2 is the Signature Facial promo (price 150 pounds, was 210 pounds, CTA to /book?treatment=face-treatments -- the Signature Facials & Skin treatment slug in lib/treatments.ts, same query-param pattern components/booking/BookingButtons.tsx already uses). components/home/HeroCarousel.tsx -- a small "use client" shell that owns only the interval timer, pause-on-hover/focus and the dot controls; it receives pre-rendered slide content as children rather than re-rendering slide markup itself, so slide 1 still paints server-side with no hydration wait. components/home/Hero.tsx -- rewritten to build each slide\'s JSX (BrandSlide = the original hero content, unchanged; PromoSlide = the new promo layout, same dark ink + radial gold glow + KMark language as the original hero and components/academy/AcademyBanner.tsx) and hand the array to HeroCarousel. Hero\'s exported signature (Hero({ rating })) is unchanged, so app/(marketing)/page.tsx needed no edit.',
      'Rotation: 7.5s auto-advance, paused for as long as the pointer or keyboard focus is anywhere inside the hero (blur only resumes once focus actually leaves the hero, not when it moves between two elements inside it). Dots are real <button>s with aria-label "Show slide N of M: <slide name>" and aria-current on the active one -- keyboard-operable, no focus trap (inert on the non-active slide keeps its links/buttons out of both the tab order and hit-testing while it is faded out). The section carries aria-roledescription="carousel" and an aria-label only when there is more than one slide. Auto-rotation is fully disabled under prefers-reduced-motion (useReducedMotionSafe); the crossfade is a plain CSS opacity transition, which app/globals.css already collapses to near-zero duration for every element under the same media query, so reduced-motion users get an instant, motion-free swap instead of a second transition system to maintain.',
      'Verified: npx tsc --noEmit and DATABASE_URL= npx next build both pass clean. Branch claude/homepage-hero-carousel-1197, no PR opened yet -- pushed for review.',
    ],
  },
];
