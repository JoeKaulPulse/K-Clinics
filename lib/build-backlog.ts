...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  {
    title: 'Rotating promotional hero banner (BLD-1197)', type: 'TASK', urgency: 'P1', status: 'IN_REVIEW', assignee: 'claude',
    value: 6, effort: 3,
    detail: 'Owner request (info@kclinics.co.uk) via the admin board: the homepage hero only ever shows one static slide; the owner wants it to rotate between the existing branding slide and promotional slides for specific treatments (Signature Facial at launch -- GBP150, was GBP210 -- with Laser Hair Removal, HydraFacial Platinum, HIFU and seasonal offers named as future slides).',
    notes: [
      'components/home/hero-slides.tsx: new ordered HERO_SLIDES array (a "brand" slide plus any number of "promo" slides) so future promotions are a one-object append, no other file touched. components/home/HeroCarousel.tsx: new small client component owning only the interactive bits -- interval timer (7.5s), pause on hover/focus, dot controls with per-dot aria-label ("Show slide N of M: <name>"), aria-current, and a CSS opacity crossfade. Auto-rotation is fully disabled under prefers-reduced-motion (existing useReducedMotionSafe hook) and non-visible slides get inert + aria-hidden so they are unreachable by keyboard/AT. components/home/Hero.tsx: unchanged external signature (still `<Hero rating={...}>` from app/(marketing)/page.tsx, no call-site change needed) but now renders each slide\'s markup server-side -- slide 1 is the original static hero content verbatim, unchanged copy/CTAs/rating strip -- and hands the pre-rendered nodes to <HeroCarousel>, so the first frame still paints without waiting on client hydration.',
      'The Signature Facial slide follows the owner-supplied copy verbatim (eyebrow, title, tagline, description, GBP150/was GBP210/limited-time note) and reuses the site\'s existing dark ink + radial gold-glow + KMark + Fraunces treatment (glow re-anchored top-left so it does not read as a repeat of slide 1), staying within the brand palette tokens with no plain-text logo/strapline. Its CTA links to site.booking.path with a ?treatment=face-treatments query param, the same pattern BookingButtons.tsx already uses to pre-select a treatment in the booking flow; a secondary link points at the existing /face-treatments page.',
      'Pre-merge review, driven in headless Chromium against a local production build at 1440x900 and 390x844. Behaviour verified, not just read: left alone the hero advances on its own; with the pointer anywhere over it, it does not; with keyboard focus inside it and the pointer never used, it does not; after pressing Pause it stays stopped even once focus and pointer leave; and the faded-out slide exposes zero focusable controls (inert holds). Slide 1 was diffed against origin/main line by line -- the only changes are the wrapper element (section moved up into HeroCarousel) and the scroll cue moving from bottom-8 to bottom-20 to clear the dots. Copy, both CTAs and the rating strip are untouched. The CTA target was checked live: /book?treatment=face-treatments resolves to the real, bookable "Signature Facials & Skin" service (aesthetics, so not behind the dentistry gate), and /face-treatments returns 200. Brand check passes -- palette tokens only, Fraunces on display type, the K monogram rendered from components/brand/marks.tsx as a decorative aria-hidden mark, and no strap-line or typed brand name anywhere in the slide.',
      'Three defects found and fixed on the branch. (1) WCAG 2.2.2 (Level A): the hero rotated automatically with hover and focus as the only way to stop it, which leaves touch users no mechanism at all -- added an explicit Pause/Resume control in the dot pill (hidden under prefers-reduced-motion, where nothing rotates). (2) WCAG 2.5.8 (AA, target size): the dots were 8x8 px with 10 px gaps, so neither the 24 px minimum nor the spacing exception was met -- each dot is now a 24x24 px button wrapping the same small visual pill; measured 24x24 in the browser. (3) Hover and focus shared one `paused` boolean, so moving the mouse off the hero restarted rotation while a dot still had keyboard focus -- they are now separate flags, and pressing a dot restarts the 7.5s dwell instead of letting the slide change a fraction of a second later. Also: the "Was GBP210" line and the price note were 55% porcelain, which measures about 3.8:1 against ink under the slide\'s gold glow and fails AA; both are now 65% (4.7-5.7:1 across the slide).',
      'NOT fixed, needs the owner before this ships: the slide advertises the Signature Facial at GBP150, was GBP210, as a limited-time introductory price, and nothing on the live site supports either number. The live catalogue prices face-treatments at GBP125 for the single 60-minute Full Face session (course of 3 at GBP320, GBP106.67 a session), so the hero offer is GBP25 dearer than the price the treatment page shows, and no GBP210 has ever been listed. A "was" price has to be one genuinely charged before. Prices are a business fact, so no code guess was made here: either the owner confirms the figures and the admin catalogue/offer is updated to match, or the slide copy changes. Until then the homepage and /face-treatments contradict each other in public.',
      'Left as owner design calls, not changed: the K monogram on the promo slide sits at 110% height and 35% opacity against slide 1\'s 80%/55%, so it reads noticeably heavier; the scroll cue and the dot pill now stack as two rounded controls at the bottom centre on desktop; and while slide 2 is showing, the page\'s only h1 is inside the inert slide 1, so assistive tech sees no h1 during that window (legal under WCAG, and making the promo heading a second h1 would put a competing page heading in the homepage markup -- worth an SEO opinion rather than a silent change).',
    ],
  },
];
