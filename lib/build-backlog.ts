...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  },
  {
    ...
    notes: ['Fix: added await to both fetch(webhookUrl, ...) calls (app/api/cron/daily/route.ts, app/api/health/route.ts), matching the pattern already used in app/api/cron/dispatch/route.ts and app/api/cron/kiosk-cleanup/route.ts. Both enclosing route handlers were already async, so no other change was needed. Each awaited POST carries signal: AbortSignal.timeout(5_000) (the same bounded-outbound-call pattern as lib/sms.ts and lib/ga4-data.ts) so a hung webhook cannot stall /api/health, which has no maxDuration override and runs on a 5-minute cron. tsc passes; DB-dependent prebuild steps could not run in the sandbox (no reachable Postgres), so npm run build was verified with DB_SYNC_NONFATAL=true to isolate the Next.js compile from that unrelated sandbox limitation -- the change carries no schema changes. (BLD-1137)'],
  },
  {
