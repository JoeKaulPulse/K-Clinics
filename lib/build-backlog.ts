...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  },
  {
    ...
    value: 7, effort: 3,
    detail: 'After its initial Promise.all, app/admin/reports/page.tsx separately awaited staffRows, goodsCost, usedCost, minMarginPct, vatCfg, usedRow, and an appointmentSession query one after another. All but staffRows depend only on the already-known since/bookingWhere and do not need to wait on each other.',
    notes: ['Fix: batched goodsCost, usedCost, minMarginPct, vatCfg, usedRow and the appointment-session query into one Promise.all in app/admin/reports/page.tsx -- all six only close over since/bookingWhere, established before the first Promise.all. staffRows stays sequential (it depends on staffIds, derived from byStaff in the first batch). The VAT block\'s own nested bySlug/svcRows Promise.all is unchanged, now just fed by the pre-fetched vatCfg. No output changes -- same data, fewer round trips. (BLD-1126)'],
  },
  {
    ...
    value: 7, effort: 3,
    detail: 'In app/api/stripe/webhook/route.ts\'s charge.refunded handler (booking branch), a refund issued directly in the Stripe dashboard reconciles refundedPence, claws back loyalty points, pushes the Xero credit note and emails the client -- but never calls creditVoucher to restore booking.giftVoucherPence back onto the voucher, unlike the in-app refundBooking() in lib/booking-actions.ts (BLD-882) which explicitly does this on a full refund. A booking partially settled by card + gift voucher, refunded via the Stripe dashboard instead of the admin UI, permanently strands the voucher-covered value.',
    notes: ['Fix: added the same guarded creditVoucher call inside the webhook\'s existing `if (fully)` branch (app/api/stripe/webhook/route.ts) -- gated on chargePaymentIntentId !== \'ext_gift-voucher\' && giftVoucherPence > 0 && giftVoucherCode, mirroring refundBooking()\'s exact condition, plus a matching REWARD_REDEEMED audit entry. creditVoucher is idempotent (capped at the voucher\'s face value), and this only runs on the CAS-winning full-refund path, so a redelivered webhook event cannot double-credit. (BLD-1138)'],
  },
];
