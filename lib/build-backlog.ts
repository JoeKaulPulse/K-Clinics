...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  {
    title: 'ChatMessage.body stored in plaintext, bypassing the app\'s clinical-encryption pattern', type: 'ERROR', urgency: 'P1', status: 'IN_REVIEW', assignee: 'claude',
    value: 7, effort: 3,
  ...
  {
    title: 'Finance batch: BNPL phantom-debt rebooking block, unreconciled voucher revenue, undercounted voucher loyalty spend', type: 'ERROR', urgency: 'P1', status: 'IN_REVIEW', assignee: 'claude',
    value: 7, effort: 3,
    detail: 'Three finance/voucher bugs: a client who pre-paid a course via BNPL then late-cancelled was reported as owing the full course price and blocked from rebooking; a booking settled entirely by gift voucher never raised a Xero sales invoice, so that revenue was invisible in the books; and a partial-voucher booking earned loyalty points and K Circle tier credit only on the card remainder, undercounting the voucher-covered portion of the same spend.',
    notes: [
      ...
      'Fix (BLD-1201): the \'voucher\' full-cover settlement branch in app/api/admin/bookings/session/route.ts wrote the booking straight to PAID (chargedAt/chargedPence set) with no Xero push, unlike the card-charge paths in lib/booking-actions.ts (chargeBooking, finalizeBookingCharge), which both call pushBookingSaleToXero after a successful charge. Only the \'external\' branch in the same file is meant to skip Xero (documented in its own comment, since an outside platform owns that reconciliation) -- a voucher redemption is genuine in-house revenue. Added the same pushBookingSaleToXero(bookingId) call (dynamic import, try/catch with Sentry capture) to the voucher full-cover branch, right after its awardClientSpend call.',
