...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  // title and advances the live item to SHIPPED on the next board load.
  {
    ...
    value: 9, effort: 3,
    detail: '@sentry/nextjs 10.x integrated. instrumentation.ts (register + onRequestError = captureRequestError) covers server-side errors in route handlers, Server Components and middleware. instrumentation-client.ts covers client-side errors (this file name is the convention Turbopack injects — the project builds with Turbopack, so the legacy sentry.client.config.ts would not load). global-error.tsx now calls captureException on render. CSP updated to allow *.sentry.io + *.ingest.sentry.io in connect-src. withSentryConfig wraps next.config.mjs conditionally (only active when SENTRY_DSN or SENTRY_AUTH_TOKEN is in the build env) so builds without Sentry are unchanged. To activate: add SENTRY_DSN (server) and NEXT_PUBLIC_SENTRY_DSN (client) to Vercel env vars and redeploy. Optionally add SENTRY_AUTH_TOKEN for source-map upload.',
    notes: ['instrumentation.ts: register() + onRequestError = Sentry.captureRequestError. instrumentation-client.ts: client init + onRouterTransitionStart (no-op if NEXT_PUBLIC_SENTRY_DSN unset; Turbopack-compatible file name). next.config.mjs: withSentryConfig conditional on hasSentry = Boolean(SENTRY_DSN || SENTRY_AUTH_TOKEN). CSP: connect-src += *.sentry.io + *.ingest.sentry.io. Alert thresholds: configure in Sentry dashboard under Project > Alerts after first errors arrive. tracesSampleRate: 0.05 (5% of requests traced).'],
  },
  {
