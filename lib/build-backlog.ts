...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  },
  {
    ...
    notes: [
      ...
      'Fix (BLD-1289): added the /i flag to stripNavChrome()\'s leading-nav-label regex in lib/blog.ts (the shape it matches -- a leading run of 1-3 Capitalized-ish words + "Blog" -- is unchanged; /i only affects letter-casing, not which characters match, so a legitimate excerpt that happens to start with a capitalized word is still left untouched exactly as before). Re-verified against both capitalized ("Dentistry Blog") and lower-case ("Dentistry blog") nav-label text, plus the untouched-prose negative case (a "Microneedling is a collagen induction therapy..." style excerpt with no nav chrome).',
