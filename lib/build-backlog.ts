...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  {
    // Title matches the live board card exactly so seedBacklog dedupes onto it.
    title: 'Server-side ad-conversion sends use a stale consent snapshot instead of live consent', type: 'ERROR', urgency: 'P1', status: 'BLOCKED', assignee: 'claude',
    value: 8, effort: 3,
    ...
    notes: ['Attempted fix (reverted in review, see next note): added liveConsent() to lib/marketing.ts, which re-reads the kc_analytics_consent/kc_marketing_consent cookies at send time via cookies() (next/headers), wrapped in the same try/catch + fail-closed-to-false pattern as bookingAttribution() -- mirroring the live-cookie-read approach app/api/finder-lead/route.ts already used via consentFromCookieHeader(). All 5 charge/refund conversion-send call sites across the 4 files now call liveConsent() for the analyticsConsent/marketingConsent gate instead of reading the stored Booking/Order/Enrolment column. The stored columns are left in place and still written at creation time -- they remain the record of consent-at-purchase-intent and are used elsewhere (attribution reporting, admin client actions); only the send-time consent GATE changed. (BLD-1880)',
      'Review: the liveConsent() change above was reverted before merge; no code change ships for BLD-1880. cookies() reads the consent of whoever made the current request, not the client being charged. finalizeOrder() runs from the admin POS and orders routes, finalizeBookingCharge() from the staff terminal route, chargeBookingAction() and refundBooking() from staff sessions, so a staff browser that had accepted cookies would have sent Purchase/Refund events for in-clinic customers who never saw the banner (fail-open). Every Stripe-webhook send would also have been suppressed, because webhooks carry no cookies. The stored Booking/Order/Enrolment snapshot (default false) remains the gate, which is the PRJ-1034.4 design.',
      'Why BLOCKED: the banner withdraws consent by deleting the cookie, so a withdrawal cannot be told apart from a request with no visitor cookies. Honouring withdrawal after booking needs a server-side consent record the deferred send can read, for example consent changes written against the client or a first-party visitor id, and checked as snapshot AND not-withdrawn. Owner decision needed on that design. docs/data-protection/subject-rights.md should not claim server-side sends stop live until then.',
    ],
  },
  {
    // Title matches the live board card exactly so seedBacklog dedupes onto it.
  ...
  },
  {
    ...
    notes: [
      "Fix: removed the ambient auditClinicalView() call from app/admin/page.tsx's next-arrival block. The card's allergy/medical-flag chip now only states that a flag is on file; the dashboard render no longer decrypts or ships the text to the browser.",
      "Review follow-up (same ticket): removing the audit call on its own left the dashboard decrypting and displaying Art. 9 text with no audit row, against the rule in lib/clinical-view-audit.ts (every display-time decrypt is audited). The chip now has a Show button that calls a new server action, revealArrivalClinical() in app/admin/arrival-actions.ts. It requires bookings.view + clients.clinical.view, applies the PRACTITIONER own-booking scope used by getBooking(), decrypts, and writes the audit row with surface 'admin-dashboard-reveal', so every logged view now matches a real tap. Revealed text is keyed to the booking id so a dashboard refresh cannot show one client's flags under another client's name. Historical 'admin-dashboard' rows are the spurious ones; new rows use the new surface name.",
      "Tasks board (app/admin/tasks/page.tsx, surface 'tasks-board'): deliberately left audited. Unlike the dashboard, the board renders each clinical task's decrypted detail inline, so its rows record a real disclosure; deleting that call would under-log. The fix there, if wanted, is the same reveal-on-demand pattern in TaskBoard, tracked as a follow-up.",
      "Verified: npx tsc --noEmit and DB_SYNC_NONFATAL=true npm run build both pass clean (this sandbox cannot reach the production Postgres host over raw Postgres, so the prebuild db-sync step is expected to fail here -- DB_SYNC_NONFATAL is the existing opt-in for exactly that, used for local verification only, not set anywhere in committed config).",
    ],
  },
  {
    title: "Guest checkout shop orders are invisible to both erasure and the client's own data export",
    type: 'ERROR', urgency: 'P1', status: 'SHIPPED', assignee: 'claude', pr: PR(2001),
    value: 7, effort: 2,
  ...
  },
  {
    ...
    notes: [
      ...
      "Review follow-up (same ticket): app/admin/incidents/page.tsx rendered the same clinic-wide register server-side via listIncidentRegister(), bypassing the API gate, so a PRACTITIONER could still read every client's incident detail through the UI. The page now requires compliance.manage as well as clients.clinical.view (same gate as ?all=1), and its nav entry in lib/admin-nav.ts is gated on compliance.manage so practitioners no longer see a link that redirects them.",
      "Review follow-up (same ticket): the sibling app/api/admin/bookings/before-photo/route.ts had the same hole on write. POST now refuses (404) a PRACTITIONER attaching a photo to a booking that is not theirs (booking.practitionerId, same rule as getBooking), and DELETE applies the same client-ownership check as GET before-photo/[id] before deleting.",
  ...
  },
  {
    ...
    notes: [
      ...
      "Review follow-up (same ticket): the same losing text-[var(--color-gold-soft)] class sat on 10 more dark-surface eyebrows, including PageHero (every interior marketing page), TreatmentTemplate (every treatment page) and NewsletterCapture (site-wide). Added one modifier, .eyebrow-on-dark in app/globals.css, which sets --eyebrow-color: var(--color-gold-soft), and applied it to all 14 dark eyebrows: HeroSlider (4, replacing the inline style casts), PageHero, TreatmentTemplate, NewsletterCapture, PinnedExperience, AcademyBanner, DashboardHero, the homepage card eyebrow, academy page, not-found and marketing error page. Each surface was checked as dark (ink, surface-ink, dark GenerativeArt or an ink overlay under porcelain text). Contrast re-computed: gold-soft #c2a589 on ink #2a2420 is 6.59:1; under the hero's gold glow at the eyebrow's position (about 20% gold mix at most) it is at least 4.9:1.",
      "Verified: npx tsc --noEmit and npm run build both pass clean.",
    ],
  },
  {
    ...
    notes: [
      ...
      "Marked SHIPPED as verified already fixed, no code change (same convention as the 2026-08-09 audit re-check entry). Review re-derived it from source rather than live HTML: components/ui/PageHero.tsx always renders WordReveal with as=\"h1\" and takes no prop to change it; all 11 routes render PageHero on their default path; the homepage's HeroSlider slide 1 is the only <h1> (slides 2-4 use <h2>). The CMS-branch caveat above stands: a published CMS page for about/contact/membership gets an <h1> only if it includes a hero section (SectionRenderer renders that through PageHero).",
      "Verified: npx tsc --noEmit and npm run build both pass clean.",
    ],
  },
];
