...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  {
    // Title matches the live board card exactly so seedBacklog dedupes onto it.
    title: 'URGENT: System-Wide Appointment Time Mismatch', type: 'ERROR', urgency: 'P0', status: 'IN_REVIEW', assignee: 'claude',
    ...
    notes: ['Fix: added `timeZone: CLINIC_TZ` (Europe/London) to every server-side date/time format call missing it: the same-day booking request notification + audit summary (app/api/booking/start/route.ts), booking cancelled/rescheduled staff notifications and the reschedule interaction summary (lib/booking-actions.ts), the in-session next-visit audit summary (app/api/admin/bookings/session/route.ts), and the client-facing appointments list including a raw `.getDate()` call that read the server-local (UTC) day-of-month instead of the clinic-local one (app/account/appointments/page.tsx). No change to how times are created or stored -- clinicWallTimeToUTC/availability were already correct; this was purely a display/notification-formatting gap. typecheck + production build both green.'],
  },
];
