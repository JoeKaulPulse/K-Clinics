...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  {
    title: 'Gift-voucher batch: cancel/reschedule fees skip the voucher discount, a lost dispute strands reserved balance, abandoned shop orders never release it', type: 'ERROR', urgency: 'P1', status: 'IN_REVIEW', assignee: 'claude',
    value: 7, effort: 3,
    detail: 'Three independent gift-voucher accounting bugs, all confirmed against current code: cancelBooking/rescheduleBooking billed the late-cancellation/reschedule fee on the pre-voucher price, on top of a discount the client already paid for with a voucher; a Stripe dispute lost on a shop order never credited its reserved gift-card balance back; and app/api/shop/checkout creates a plain PaymentIntent (not a Checkout Session, which auto-expires), so a customer who never confirms payment leaves the Order PENDING forever with its reserved gift-card balance stranded -- no payment_intent.canceled event ever fires for it.',
    notes: [
      'Fix (BLD-1236): cancelBooking()\'s chargeablePence and rescheduleBooking()\'s rescheduleFeePence (lib/booking-actions.ts) netted off pointsRedeemedPence only, never giftVoucherPence, so a partially-voucher-paid booking was billed its late-cancellation or 4th+ reschedule fee on the full pre-voucher price. Both now also subtract booking.giftVoucherPence, matching the pattern already used by the staff "charge now" action (app/admin/bookings/actions.ts, BLD-882) and by courseTotalPence() in the same file.',
      'Fix (BLD-1237): the charge.dispute.closed lost-dispute order lookup in app/api/stripe/webhook/route.ts selected only { id, number } and its handler did status: REFUNDED + restockOrder with no gift-card step, unlike the charge.refunded branch a few cases below. Added giftCardCode/giftCardPence to both the initial lookup and the fullOrder re-select, and call creditVoucher(giftCardCode, giftCardPence) alongside the restock, guarded the same way (only when a gift card was applied) as the refunded branch.',
      'Fix (BLD-1253): added releaseAbandonedPendingOrders() to lib/automations.ts -- a new, unconditional (not gated behind the abandoned_order_recovery setting, which only controls the reminder email) sweep, separate from the existing abandonedOrders() reminder job. It finds PENDING Orders with a PaymentIntent older than 7 days, re-confirms each PaymentIntent\'s live Stripe status first (skipping any that in fact succeeded/are mid-confirmation, mirroring the caution app/api/admin/pos/route.ts\'s cancel op already takes), then atomically claims PENDING->CANCELLED and calls undoVoucherReservation() for any reserved balance -- the same function the POS cancel path uses -- and best-effort cancels the PaymentIntent itself. The 7-day cutoff is deliberately longer than releaseAbandonedPendingBookings()\'s 45-minute window: abandonedOrders() emails a "finish your order" reminder 2-72h after checkout, and releasing before that window closes would cancel the order out from under its own reminder link. Wired into the existing 15-minute app/api/cron/dispatch sweep alongside releaseAbandonedPendingBookings. abandonedOrders() itself is unchanged.',
    ],
  },
];
