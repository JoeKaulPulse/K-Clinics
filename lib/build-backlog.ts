...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  },
  {
    ...
    value: 8, effort: 3,
    detail: 'The ability to override the treatment price should also be available on the main Appointment page, not just within the Live Appointment view. Staff need to edit the treatment price directly from the Appointment page, applying only to that specific appointment, with the default treatment pricing unchanged. The same override functionality that previously existed should be restored.',
    notes: ['Scoped narrower than the ask, deliberately: the Live Appointment checkout\'s "override" is a free-editable charge amount at the point of taking payment -- it never rewrites Booking.pricePence, and BLD-1094 (owner-gated, still in TRIAGE) already flags that repricing an appointment which has touched a real Stripe charge, Xero invoice, loyalty points or a day-close snapshot needs an owner call on what else that should re-trigger. This ships the safe subset: a new admin-only "Override price" control on the main Appointment page (components/admin/BookingActions.tsx), visible whenever a booking has NOT yet been charged or pre-paid (upcoming or completed-but-unbilled), that edits Booking.pricePence directly for that one booking. Added updateBookingPriceAction (app/admin/bookings/actions.ts), gated on sessionIsAdmin -- matching the existing admin-only price override at booking creation (BLD-812) -- and refusing once chargedAt/prepaidAt is set. Every downstream figure (invoices, client account, the existing "Charge card on file" amount) already reads pricePence live, so this reaches them without touching any charge/refund/Xero/loyalty code. Logs a new BOOKING_PRICE_OVERRIDDEN audit action (additive Prisma enum value) plus an Interaction note with the before/after price and reason. Does NOT touch already-charged/completed-and-paid bookings -- that remains BLD-1094\'s owner decision. (BLD-1149)',
      'Pre-merge review hardening: (1) the price write is now a guarded updateMany (id + chargedAt: null + prepaidAt: null + the pricePence we read), so a charge finalised on another screen -- or by the Stripe webhook -- between the read and the write can no longer reprice a booking whose money has moved, and a concurrent add-on edit is no longer clobbered; staff get a reload-and-retry message instead. (2) app/api/admin/bookings/bnpl-link now also refuses when Booking.pricePence is BELOW the primary line item: courseTotalPence reads that item (deliberately, so an override cannot leak into a rebooked next visit), so a BNPL pre-payment link raised after a downward override would have billed the client the pre-override course total and marked it paid in full. (BLD-1149)'],
  },
];
