...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  {
    title: 'Academy quiz/flashcard images carry no alt text; student erasure leaves academy PII behind', type: 'ERROR', urgency: 'P1', status: 'IN_REVIEW', assignee: 'claude',
    value: 7, effort: 2,
    detail: 'Two related academy correctness bugs. (1) QuizQuestion.imageUrl and Flashcard.imageUrl (anatomy landmarks, injection sites -- the content being tested, not decoration) were rendered with alt="" in components/academy/ImmersiveCourse.tsx, components/academy/CoursePlayer.tsx and components/academy/ReviseBoard.tsx, and the schema had no field to hold a description -- a screen-reader trainee could not perceive image-based questions at all (WCAG 1.1.1). (2) eraseStudentData() in app/admin/actions.ts nulled the student\'s own identity/credentials but left AcademyStudent.notes/goals, FundingApplication name/email/phone, Enrolment applicant fields and HomeworkSubmission note/feedback/files untouched, and logged the action as the generic NOTE_ADDED audit type instead of a distinct erasure action -- so a GDPR Art.17 erasure request for a trainee was materially less complete than the equivalent eraseClientData() path.',
    notes: ['Fix (1): added an optional QuizQuestion.imageAlt / Flashcard.imageAlt string column (additive, deploy-gate safe) and threaded it through app/api/admin/lms/route.ts (updateQuestion) and app/api/admin/flashcards/route.ts (updateCard), lib/lms.ts and lib/flashcards.ts\'s view types/selects, the admin editors (components/admin/CurriculumManager.tsx, components/admin/FlashcardsManager.tsx -- a new "Image description" field appears once an image is attached) and app/admin/academy/[courseId]/page.tsx\'s server-side view mapper. The three trainee-facing render sites now use alt={q.imageAlt || \'Course image\'} / alt={card.imageAlt || \'Course image\'} instead of alt="" -- existing images with no description yet fall back to a generic, non-empty alt rather than being silently skipped by a screen reader. Fix (2): eraseStudentData() now also nulls AcademyStudent.notes/goals; redacts Enrolment.applicantName/applicantEmail/applicantPhone for the student\'s enrolments; redacts FundingApplication name/email/phone both by studentId and, mirroring eraseClientData()\'s email-matched guest records, by the original email for pre-account funding enquiries; and nulls HomeworkSubmission.note/feedback and clears its files array (row kept, matching eraseClientData()\'s redact-not-delete pattern for retained records) -- same $transaction, same field-nulling approach as eraseClientData(), no rows deleted. Added a distinct STUDENT_ERASED value to the AuditAction enum (additive) and switched the audit log call to it from the generic NOTE_ADDED. npx tsc --noEmit and npm run build both pass. (BLD-1140, BLD-1124)'],
  },
];
