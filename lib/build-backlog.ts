...
export const BUILD_BACKLOG: BacklogItem[] = [
  ...
  },
  {
    ...
    notes: ['Fix: components/academy/PracticeRunner.tsx now accumulates a per-question answers array ({questionId, correct} from each server-graded \'check\' response) in state and sends that array (not total/correct) on \'submit\'. app/api/academy/practice/route.ts validates each entry (string questionId, boolean correct), caps the array at 30, and deduplicates by questionId with a Set so replaying the same questionId cannot inflate the count; total and correct are then derived server-side from the deduplicated set before calling recordPractice. recordPractice\'s own signature is unchanged. Residual limitation (noted for the audit/PR): this is a dedup-based mitigation, not a full replay-proof fix -- it removes the trivial total:100,correct:100 abuse case but a client could still fabricate correct:true against questionIds it never actually answered without also faking a matching \'check\' call, since the server doesn\'t re-verify the per-question correctness at submit time. Full end-to-end proof would need server-side session state tracking which questions were actually served and checked, which is out of scope for this pass. (PRJ-1043.11)'],
  },
];
