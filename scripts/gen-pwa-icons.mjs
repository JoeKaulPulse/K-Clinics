// PRJ-1032.37: rasterise the brand icon to the PNG sizes a PWA install needs.
// The manifest previously shipped only /icon.svg, so Android had no maskable or
// raster icon for the home-screen/splash. This renders, from the SAME K monogram
// mark in public/icon.svg:
//   • icon-192.png, icon-512.png  — purpose "any" (the rounded-square brand tile)
//   • icon-maskable-512.png       — purpose "maskable": full-bleed background with
//     the mark inside the central 80% safe zone, so an OS mask (circle/squircle)
//     never clips the K or exposes a transparent corner.
// Idempotent — safe to re-run. Run: node scripts/gen-pwa-icons.mjs
...
// The K path from public/icon.svg (translate(65.5,13.5) in a 260×260 view).
const K_PATH =
...
// "any" icons are the exact brand tile from public/icon.svg.
const svg = readFileSync(join(PUBLIC, 'icon.svg'));

// Maskable: full-bleed INK background (no rounded corners — the OS supplies the
// mask) with the mark scaled into the central safe zone (~64% of the canvas).
// The mark art is ~128 wide × 233 tall in its own space; centre it on 260×260.
const maskableSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 260 260">
  <rect width="260" height="260" fill="${INK}"/>
  <g transform="translate(94,30) scale(0.72)"><path fill="${SAND}" d="${K_PATH}"/></g>
</svg>`;

async function png(input, size, out) {
  await sharp(input, { density: 384 }).resize(size, size, { fit: 'contain' }).png().toFile(join(PUBLIC, out));
  console.log(`[pwa-icons] wrote public/${out} (${size}×${size})`);
}

await png(svg, 192, 'icon-192.png');
await png(svg, 512, 'icon-512.png');
await png(Buffer.from(maskableSvg), 512, 'icon-maskable-512.png');
