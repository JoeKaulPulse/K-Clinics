...
render([
  ...
  { h2: 'Google reviews — on your site & the “leave a review” link' },
  { p: 'Two settings: one sends happy clients to Google to leave a review, the other pulls your real Google star-rating and reviews onto the site (homepage, reviews page), blended honestly with your in-house reviews.' },
  { steps: [
    'Find your clinic’s “Place ID” using Google’s Place ID finder, and paste it into Vercel as GOOGLE_PLACE_ID (this powers the “Share it on Google” link).',
    'In the Google Cloud Console, enable the “Places API”, then under “Credentials” create an “API key”.',
    'Restrict that key to the Places API (Credentials → your key → API restrictions), then copy it.',
    'Paste it into Vercel as GOOGLE_PLACES_API_KEY and redeploy — your Google rating + reviews now show on the site.',
  ] },
  { link: ['Google — Place ID finder', 'https://developers.google.com/maps/documentation/places/web-service/place-id'] },
  { link: ['Google — create & restrict an API key', 'https://developers.google.com/maps/documentation/places/web-service/get-api-key'] },
  { table: [['Paste into Vercel as', 'Value'], [
    ['GOOGLE_PLACE_ID', 'ChIJ… (your Place ID)'],
    ['GOOGLE_PLACES_API_KEY', 'the Places API key'],
  ], [44, 56]] },
  { tip: 'Without GOOGLE_PLACES_API_KEY only your in-house reviews feed the public rating — no Google reviews appear on the site.', label: 'Tip' },
  { h2: 'Read & reply to every Google review (in the admin)' },
  { p: 'A deeper connection lets you see ALL your Google reviews inside the admin Reviews page and reply to them without leaving the dashboard. It uses the same Google login as the calendar, plus one extra approval from Google.' },
  { steps: [
    'In the Google Cloud Console, request access to the “Business Profile APIs” (a short form — approval can take a few days) and enable the “My Business” / Business Profile APIs.',
    'In your OAuth client (the same one as the calendar), add a SECOND authorised redirect URI: https://kclinics.co.uk/api/admin/integrations/google-business/callback',
    'Find your Business “Account ID” and “Location ID” (Business Profile → the numbers in the page address, or via the API) and paste them into Vercel as GOOGLE_BUSINESS_ACCOUNT_ID and GOOGLE_BUSINESS_LOCATION_ID. Redeploy.',
    'In Admin → Reviews, scroll to “Google reviews” and click “Connect Google Business”, then sign in once.',
    'Click “Sync now” to import. From then on they refresh automatically every night, and you can reply to any review in place.',
  ] },
  { link: ['Google — Business Profile API access', 'https://developers.google.com/my-business/content/prereqs'] },
  { table: [['Paste into Vercel as', 'Value'], [
    ['GOOGLE_BUSINESS_ACCOUNT_ID', 'your account number'],
    ['GOOGLE_BUSINESS_LOCATION_ID', 'your location number'],
  ], [46, 54]] },
  { tip: 'This add-on is the only Google piece that needs Google to approve API access first, so start that request early if you want in-admin replies — everything else works immediately.', label: 'Tip' },
...
render([
  { table: [['Setting name (in Vercel)', 'What it’s for', 'Step'], [
    ...
    ['GOOGLE_PLACE_ID', 'Google review link', '14'],
    ['GOOGLE_PLACES_API_KEY', 'Show Google reviews on site', '14'],
    ['GOOGLE_BUSINESS_ACCOUNT_ID / _LOCATION_ID', 'Reply to Google reviews in admin', '14'],
