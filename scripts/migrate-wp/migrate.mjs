...
//
// Also callable in-process (the /api/build/migrate-wp runner):
//     await run({ file, commit, refresh, log })
//
...
export async function run({ file, commit = false, refresh = false, log = console.log } = {}) {
  ...
  log(`Source rows scanned : users=${num(users.size)}  usermeta-users=${num(meta.size)}  legacy-orders=${num(orderMeta.size)}  hpos-addr=${num(hposAddr.length)}  lookup=${num(lookup.length)}`);
  log(`Contributed         : from users=${num(fromUsers)}  from orders=${num(fromOrders)}  skipped(no email)=${num(skippedNoEmail)}`);
  log(`\n→ UNIQUE CLIENTS (deduped by email): ${num(list.length)}`);
  log(`   with phone   : ${num(withPhone)}`);
  log(`   with DOB     : ${num(withDob)}`);
  log(`   with address : ${num(withAddr)}  (kept in notes until we add address fields)`);
  log(`   marketing opt-in (explicit) : ${num(optedIn)}`);
  log(`   names needing review (no real name in source): ${num(needsReview)}  ${refresh ? '[--refresh: messy names will be overwritten]' : ''}`);
  log(`   likely test/junk accounts (tagged 'likely-test'): ${num(likelyTest)}`);

  if (unmapped.size) {
    log('\nUnmapped usermeta keys seen (PII-free key names — paste back so I can map them):');
    for (const [k, n] of [...unmapped.entries()].sort((a, b) => b[1] - a[1]).slice(0, 40)) log(`   ${String(n).padStart(6)}  ${k}`);
  }

  if (dryRun) {
    log('\nDRY RUN — nothing written. Re-run with --commit (and DATABASE_URL) to import.\n');
    return { clients: list.length, written: 0 };
  }
  ...
  try {
    for (const c of list) {
      ...
      if (n % 200 === 0) log(`  …${n}/${list.length}`);
    }
    log(`\n✓ Done. Created ${num(created)}, updated ${num(updated)}, total ${num(list.length)}.\n`);
    return { clients: list.length, created, updated };
  } finally {
    await db.$disconnect();
  }
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  await run({ file, commit: args.includes('--commit'), refresh: args.includes('--refresh') });
}
