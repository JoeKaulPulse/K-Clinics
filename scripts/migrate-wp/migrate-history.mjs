...
// (PENDING, never auto-published) instead of being dropped.
...
const repair = args.includes('--repair');   // also fix times on already-imported bookings
...
console.log(`Appointments : ${num(bookings.length)}  (cancelled ${num(bookings.filter((b) => b.status === 'CANCELLED').length)}; no matching user ${num(bkNoClient)}; time unknown ${num(bkNoTime)})`);
console.log(`Reviews      : ${num(revRecs.length)}  (unattributed → quarantine, held for moderation: ${num(revRecs.filter((r) => r.orphan).length)})`);
console.log(`Loyalty pts  : ${num(ptRecs.length)} entries  (linked to a client: ${num(ptRecs.filter((p) => p.email).length)})`);
console.log(`Services seen: ${num(service.size)}   Staff seen: ${num(staff.size)}   Time slots: ${num(slots.size)}`);
if (repair) console.log('REPAIR mode: previously imported bookings get their start/end times corrected.');
