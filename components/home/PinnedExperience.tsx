...
export function PinnedExperience() {
  ...
  return (
    <section className="surface-ink grain relative">
      ...
      {/* BLD-846: stacked layout is CSS-gated (md:hidden), not tied to the JS
          reduce flag, so standard-motion mobile visitors see the steps too —
          same pattern as HorizontalGallery's SwipeRail. */}
      <div className={`container-lux relative grid gap-10 pb-[var(--space-section)] ${reduce ? 'md:grid-cols-3' : 'md:hidden'}`}>
        {steps.map((s) => (
