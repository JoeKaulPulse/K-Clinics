...
export function PaymentMarks({ className = '' }: { className?: string }) {
  return (
    <div className={`flex flex-wrap items-center gap-1.5 ${className}`} aria-label="Accepted payment methods">
      {/* Visa */}
      <span className={wrap}><span className={`${word} italic tracking-[0.04em]`}>VISA</span></span>
      {/* Mastercard */}
      ...
      {/* American Express */}
      <span className={wrap}><span className={word}>AMEX</span></span>
      {/* Apple Pay */}
      ...
      <span className={wrap}><span className={word}>Clearpay</span></span>
      <span className={wrap}><span className={word}>Klarna</span></span>
    </div>
  );
}
