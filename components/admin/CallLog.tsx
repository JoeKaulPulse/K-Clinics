  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_22rem]">
      <div className="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-line)] bg-[var(--color-porcelain)]">
        ...
        {calls.length === 0 ? (
          <p className="p-6 text-sm text-[var(--color-stone)]">No calls yet.</p>
        ) : (
      ...
      <aside className="rounded-[var(--radius-lg)] border border-[var(--color-line)] bg-[var(--color-porcelain)] p-5">
        {!detail ? (
          <p className="text-sm text-[var(--color-stone)]">Select a call to see details, recording and transcript.</p>
        ) : (
          <div className="space-y-4">
            <div>
              ...
              {detail.agentEmail && <p className="text-xs text-[var(--color-stone)]">Handled by {detail.agentEmail}</p>}
            </div>

            {detail.recordingUrl ? (
              <audio controls preload="none" src={detail.recordingUrl} className="w-full" />
            ) : (
              <p className="text-xs text-[var(--color-stone)]">No recording for this call.</p>
            )}
            ...
            <div>
              ...
              <textarea value={note} onChange={(e) => { setNote(e.target.value); if (noteMsg) setNoteMsg(''); }} disabled={!canManage} rows={3} placeholder={canManage ? 'Add a note about this call…' : 'No note'} aria-label="Call note" className="w-full rounded-[var(--radius-sm)] border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] p-2.5 text-sm outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)] disabled:opacity-60" />
              {canManage && (
