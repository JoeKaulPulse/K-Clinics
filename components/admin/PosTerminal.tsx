...
export function PosTerminal({ products }: { products: P[] }) {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
      {/* Products */}
      <div>
        <input value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={onScan} autoFocus placeholder="Scan a barcode, or search products…" aria-label="Scan barcode or search products" className="mb-4 w-full rounded-full border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] px-5 py-3 outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]" />
        {products.length === 0 ? (
