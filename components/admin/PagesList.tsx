      ...
      <Section title="New page" hint="Create a page at a new path, built from sections.">
        <div className="px-5 py-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <input className="w-56 rounded-[var(--radius-sm)] border border-[var(--color-line)] bg-[var(--color-porcelain)] px-3 py-2 text-sm outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]" value={path} placeholder="/new-page" aria-label="New page path" onChange={(e) => setPath(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && create(path)} />
...
function ContentSearch() {
  ...
  return (
    <div className="mt-5">
      <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search page content…" aria-label="Search page content" className="w-full max-w-md rounded-full border border-[var(--color-line)] bg-[var(--color-porcelain)] px-4 py-2 text-sm outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]" />
      {searched && (
