      ...
      {open && (
        <div className="space-y-3 border-t border-[var(--color-line)] p-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={sendReply} disabled={replying || !reply.trim()} className="shrink-0 self-end rounded-full bg-[var(--color-gold)] px-4 py-2 text-sm font-medium text-white hover:bg-[var(--color-ink)] disabled:opacity-50">{replying ? 'Sending…' : 'Reply'}</button>
          </div>
        </div>
      )}
    </div>
