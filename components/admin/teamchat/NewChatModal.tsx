...
export function NewChatModal({ onClose }: { onClose: () => void }) {
  ...
  const { panelRef, onKeyDown: onDialogKeyDown } = useDialogBehaviours<HTMLDivElement>(onClose, true);
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-[rgba(42,36,32,0.5)] p-4" onClick={onClose} onKeyDown={onDialogKeyDown}>
      <div ref={panelRef} tabIndex={-1} role="dialog" aria-modal="true" aria-label="New conversation" className="w-full max-w-md rounded-[var(--radius-lg)] bg-[var(--color-porcelain)] p-5 shadow-[var(--shadow-lift)]" onClick={(e) => e.stopPropagation()}>
        ...
        {mode === 'group' && (
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Group name (e.g. Front desk)" aria-label="Group name" className="mb-2 w-full rounded-[var(--radius-sm)] border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] px-3 py-2 text-sm outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]" />
        )}
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search people…" aria-label="Search people" className="mb-2 w-full rounded-[var(--radius-sm)] border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] px-3 py-2 text-sm outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]" />
