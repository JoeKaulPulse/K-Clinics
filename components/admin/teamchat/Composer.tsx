...
export function Composer({ channel, meId, onSent, replyTo, onCancelReply }: {
  ...
  return (
    <div className="relative border-t border-[var(--color-line)] bg-[var(--color-porcelain)] px-2.5 py-2">
      ...
      {error && <p role="alert" aria-live="assertive" className="mb-1 px-1 text-xs text-[var(--color-blush-deep)]">{error}</p>}
      ...
      <div className="flex items-end gap-1">
        ...
        <textarea
          ...
          placeholder="Message…"
          aria-label="Message"
          className="max-h-28 min-h-[2.25rem] flex-1 resize-none rounded-[var(--radius-md)] border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] px-3 py-2 text-sm outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]"
        />
        <button
