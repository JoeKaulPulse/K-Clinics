      ...
      <section className="grid gap-5 md:grid-cols-[260px_1fr]">
        <div className="rounded-[var(--radius-lg)] border border-[var(--color-line)] bg-[var(--color-porcelain)] p-6 text-center">
          ...
          <div className="mt-4 flex justify-center gap-4 text-xs text-[var(--color-stone)]">
            ...
            <span><strong className="text-[var(--color-jade)]">{audit.counts.a}</strong> A-grade</span>
        ...
        <ul className="mb-4 space-y-1">
          {p.issues.map((i, idx) => (
            <li key={idx} className="flex items-start gap-2 text-xs">
              <span className={`mt-0.5 rounded px-1.5 py-0.5 text-[0.6rem] font-medium text-white ${i.severity === 'high' ? 'bg-[var(--color-blush-deep)]' : i.severity === 'med' ? 'bg-[var(--color-gold-deep)]' : 'bg-[var(--color-stone-soft)]'}`}>{i.severity}</span>
