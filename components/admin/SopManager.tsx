      ...
      <aside className="space-y-1">
        {items.map((i) => (
          ...
          >
            ...
            {i.source === 'db' ? (
              <span className={`shrink-0 rounded-full px-2 py-0.5 text-[0.6rem] ${activeSlug === i.slug ? 'bg-[var(--color-porcelain)]/20' : 'bg-[var(--color-gold)]/20'}`}>custom</span>
            ) : (
...
function Editor({ item }: { item: Item }) {
  ...
  return (
    <div className="rounded-[var(--radius-lg)] border border-[var(--color-line)] bg-[var(--color-porcelain)] p-6">
      ...
      <input value={title} onChange={(e) => setTitle(e.target.value)} className="mb-4 w-full rounded-[var(--radius-sm)] border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] px-3 py-2 text-sm outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]" />
      <label className="mb-1 block text-xs uppercase tracking-[0.14em] text-[var(--color-stone)]">Procedure steps</label>
      <textarea value={content} onChange={(e) => setContent(e.target.value)} rows={14} className="w-full rounded-[var(--radius-sm)] border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] px-3 py-2 font-[family-name:var(--font-mono)] text-sm leading-relaxed outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]" />
