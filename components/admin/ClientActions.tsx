...
const fieldCls = 'w-full rounded-[var(--radius-sm)] border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] px-3 py-2 text-sm outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]';
...
export function StatusSelect({ consultId, clientId, current }: { consultId: string; clientId: string; current: string }) {
  ...
  return (
    <select
      ...
      className="rounded-full border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] px-3 py-1 text-xs outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]"
    >
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); submit(); }} className="space-y-2 rounded-[var(--radius-md)] border border-[var(--color-line)] bg-[var(--color-porcelain)] p-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <select value={type} onChange={(e) => setType(e.target.value)} className="rounded-[var(--radius-sm)] border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] px-2 py-1.5 text-xs outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]">
...
export function PinToggle({ noteId, clientId, pinned }: { noteId: string; clientId: string; pinned: boolean }) {
  ...
  return (
    <button
      ...
      className={`text-xs transition-colors disabled:opacity-50 ${pinned ? 'text-[var(--color-gold-deep)]' : 'text-[var(--color-stone)] hover:text-[var(--color-ink)]'}`}
    >
      {pinned ? '★ Pinned' : '☆ Pin'}
    </button>
  );
