      ...
      {stage === 'enrol' && (
        <div className="mt-3 space-y-3">
          ...
          {qr
            ...
            : <p className="text-xs text-[var(--color-stone)]">(QR unavailable — use the setup key below.)</p>}
          ...
          <label className="block text-sm text-[var(--color-stone)]">2. Enter the 6-digit code it shows:
            <input value={code} onChange={(e) => setCode(e.target.value)} inputMode="numeric" placeholder="123456" className="mt-1 w-40 rounded-[var(--radius-sm)] border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] px-3 py-2 text-sm outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]" />
          </label>
