          ...
          <div ref={panelRef} role="dialog" aria-modal="true" aria-label="Report a problem" tabIndex={-1} className="my-8 w-full max-w-lg rounded-[var(--radius-lg)] bg-[var(--color-porcelain)] p-6 shadow-[var(--shadow-lift)]" onClick={(e) => e.stopPropagation()}>
            {status === 'done' ? (
              ...
              <>
                ...
                <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Short title — e.g. ‘Pricing page total is wrong’" aria-label="Title" className="mt-3 w-full rounded-[var(--radius-sm)] border border-[var(--color-line)] bg-[var(--color-porcelain)] px-3 py-2 text-sm outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]" />
                <textarea value={detail} onChange={(e) => setDetail(e.target.value)} rows={4} placeholder="What happened, what you expected, steps to reproduce…" aria-label="Details" className="mt-2 w-full rounded-[var(--radius-md)] border border-[var(--color-line)] bg-[var(--color-porcelain)] px-3 py-2 text-sm outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]" />
