...
const fieldCls = 'w-full rounded-[var(--radius-sm)] border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] px-3 py-2 text-sm outline-none transition-colors focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]';
      ...
      <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-3">
          <label className="text-xs text-[var(--color-stone)]">Category
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="ml-1.5 rounded-[var(--radius-xs)] border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] px-2 py-1 text-xs outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]">{Array.from(new Set([...CATEGORIES, category])).map((c) => <option key={c} value={c}>{c}</option>)}</select>
          </label>
