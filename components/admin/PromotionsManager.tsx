              ...
              <tbody>
                {universal.map((p) => (
                  <tr key={p.id} className="border-t border-[var(--color-line)]">
                    ...
                    <td><span className={`rounded-full px-2 py-0.5 text-[0.6rem] font-medium uppercase ${p.active ? 'bg-[var(--color-jade)]/15 text-[var(--color-jade)]' : 'bg-[var(--color-bone)] text-[var(--color-stone)]'}`}>{p.active ? 'active' : 'off'}</span></td>
