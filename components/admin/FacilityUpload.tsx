  ...
  return (
    <form ref={formRef} onSubmit={submit} className="rounded-[var(--radius-md)] border border-dashed border-[var(--color-line)] bg-[var(--color-bone)]/40 p-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <input name="title" placeholder="Title (e.g. Ground floor plan)" className={field} maxLength={160} />
        <select name="type" defaultValue="FLOOR_PLAN" className={field}>
          {TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
        </select>
        <input name="description" placeholder="Description (optional)" className={field} maxLength={1000} />
        <select name="locationId" defaultValue="" className={field}>
          <option value="">All locations</option>
          {locations.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}
        </select>
        <input name="tags" placeholder="Tags, comma-separated (optional)" className={`${field} sm:col-span-2`} />
