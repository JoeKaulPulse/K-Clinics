    ...
    <form role="search" className={`flex items-center gap-2 ${className}`}>
      ...
      <div className="group relative flex items-center">
        ...
        <input
          ...
          className={`${widthClass} h-11 rounded-full border border-[var(--color-line-strong)] bg-[var(--color-porcelain)] pl-9 pr-4 text-sm text-[var(--color-ink)] outline-none transition-shadow placeholder:text-[var(--color-stone)] focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)] focus:shadow-[0_0_0_3px_color-mix(in_oklab,var(--color-gold)_22%,transparent)] [&::-webkit-search-cancel-button]:appearance-none`}
        />
      </div>
      {showSubmit && (
