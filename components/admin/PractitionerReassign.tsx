...
export function PractitionerReassign({ bookingId, current, clinicians }: { bookingId: string; current: string | null; clinicians: ClinicianOpt[] }) {
  ...
  return (
    <div className="mt-6 flex flex-wrap items-center gap-2 text-sm">
      ...
      <select value={value} onChange={(e) => setValue(e.target.value)} className="rounded-[var(--radius-sm)] border border-[var(--color-line)] bg-[var(--color-porcelain)] px-3 py-2 text-sm outline-none focus:border-[var(--color-gold-deep)] focus-visible:ring-2 focus-visible:ring-[var(--color-gold-deep)]">
