...
export function CalendarClosureButton({ dateISO, closure }: { dateISO: string; closure: Closure | null }) {
  ...
  return (
    <div className="absolute right-0 top-12 z-30 w-72 rounded-[var(--radius-md)] border border-[var(--color-line)] bg-[var(--color-porcelain)] p-4 shadow-[var(--shadow-soft)]">
      ...
      <input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Reason (optional, e.g. Bank holiday)" aria-label="Reason" className="w-full rounded-[var(--radius-sm)] border border-[var(--color-line)] bg-[var(--color-porcelain)] px-2 py-1.5 text-sm" />
