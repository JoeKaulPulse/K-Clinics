    ...
    <article>
      ...
      <section className="relative overflow-hidden pt-[calc(var(--header-h,5.25rem)+1rem)]">
        <div className="container-lux grid gap-12 py-12 lg:grid-cols-2 lg:items-center lg:py-20">
          <div>
            <Reveal>
              <nav className="mb-6 flex items-center gap-2 text-sm text-[--color-stone]" aria-label="Breadcrumb">
                <Link href="/" className="hover:text-[--color-ink]">Home</Link>
                <span>/</span>
                <Link href={categoryHref} className="hover:text-[--color-ink]">{categoryLabel}</Link>
                <span>/</span>
                <span className="text-[--color-ink]">{t.title}</span>
              </nav>
            </Reveal>
            <Reveal delay={0.05}>
            ...
            <Reveal delay={0.15}>
              <p className="mt-4 font-[family-name:var(--font-display)] text-xl text-[--color-gold] md:text-2xl">
                {t.tagline}
              </p>
            </Reveal>
            <Reveal delay={0.2}>
              <p className="mt-6 max-w-xl text-lg leading-relaxed text-[--color-stone]">{t.intro}</p>
            </Reveal>
            <Reveal delay={0.28}>
              <div className="mt-8">
                <BookingButtons />
            ...
            <Reveal delay={0.34}>
              <dl className="mt-10 grid max-w-lg grid-cols-3 gap-4 border-t border-[--color-line] pt-6">
                {t.facts.map((f) => (
                  <div key={f.label}>
                    <dt className="text-xs uppercase tracking-[0.16em] text-[--color-stone]">{f.label}</dt>
          ...
          <Reveal delay={0.1}>
            <div className="relative">
              <GenerativeArt
                from={t.gradient[0]}
                to={t.gradient[1]}
                className="aspect-[4/5] w-full rounded-[--radius-2xl] shadow-[var(--shadow-lift)]"
              />
              {t.priceFrom && (
                <div className="card-glass absolute -bottom-5 -left-5 rounded-[--radius-md] px-6 py-4 shadow-[var(--shadow-soft)]">
                  <p className="text-xs uppercase tracking-[0.16em] text-[--color-stone]">
      ...
      {/* Benefits */}
      <section className="bg-[--color-bone] py-20 md:py-28">
        <div className="container-lux">
          <Reveal>
            <p className="eyebrow mb-4">The difference</p>
            <h2 className="text-title max-w-2xl">Why clients choose this treatment.</h2>
          </Reveal>
          <Stagger className="mt-12 grid gap-px overflow-hidden rounded-[--radius-lg] border border-[--color-line] bg-[--color-line] sm:grid-cols-2">
            {t.benefits.map((b) => (
              <StaggerItem key={b.title} className="bg-[--color-porcelain] p-8">
                <div className="mb-4 grid h-11 w-11 place-items-center rounded-full bg-[--color-ink] text-[--color-gold-soft]">
                ...
                <p className="mt-2 leading-relaxed text-[--color-stone]">{b.text}</p>
              </StaggerItem>
            ))}
          </Stagger>
        </div>
      ...
      <section className="container-lux py-20 md:py-28">
        ...
        <Stagger className="mt-12 grid gap-8 md:grid-cols-3">
          {t.process.map((s, i) => (
            <StaggerItem key={s.title} className="relative border-t border-[--color-ink] pt-6">
              ...
              <p className="mt-3 leading-relaxed text-[--color-stone]">{s.text}</p>
            </StaggerItem>
          ))}
        </Stagger>
      </section>
        ...
        <div className="container-lux grid gap-12 lg:grid-cols-[0.7fr_1.3fr] lg:items-start">
          <Reveal>
            <div className="lg:sticky lg:top-28">
              ...
              <p className="mt-5 text-[--color-stone]">
