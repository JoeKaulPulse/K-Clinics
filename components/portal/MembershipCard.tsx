    ...
    <div className="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-line)] bg-[var(--color-porcelain)] shadow-[var(--shadow-soft)]">
      <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-5" style={{ background: `radial-gradient(ellipse at 90% 0%, color-mix(in oklab, ${accent} 22%, transparent), transparent 60%)` }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[0.65rem] uppercase tracking-[0.18em] text-[var(--color-stone)]">{L('K Circle membership', 'Членство K Circle')}</p>
        ...
        <div className="text-right">
          ...
          <p className="text-xs text-[var(--color-stone)]">{L('on every visit', 'за кожен візит')}</p>
        </div>
      </div>
      ...
      <div className="px-6 py-5">
        {next ? (
          <>
            ...
            <p className="mt-2 text-xs text-[var(--color-stone)]">{L('Based on', 'На основі')} {gbp(spendPence)} {L('spent in the last 12 months. Tiers refresh on a rolling basis.', 'витрачено за останні 12 місяців. Рівні оновлюються щомісяця.')}</p>
          </>
        ) : (
          <p className="text-sm text-[var(--color-stone)]">{L('You’re at our top tier — thank you for being one of our most valued members.', 'Ви на найвищому рівні — дякуємо, що ви один із наших найцінніших клієнтів.')} <span className="text-[var(--color-stone)]">({gbp(spendPence)} {L('in 12 months', 'за 12 місяців')})</span></p>
        )}
