  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-[var(--radius-xl)] border border-[var(--color-line)] bg-[var(--color-bone)] p-7 md:p-9">
      <div className="grid gap-5 sm:grid-cols-2">
        <div><label htmlFor="gn" className={label}>Name *</label><input id="gn" name="name" autoComplete="name" aria-invalid={!!errors.name} aria-describedby={errors.name ? 'gn-err' : undefined} className={field} placeholder="Your name" onChange={() => clearErr('name')} />{errors.name && <p id="gn-err" className="mt-1.5 text-xs text-[var(--color-blush-deep)]">{errors.name}</p>}</div>
        <div><label htmlFor="gp" className={label}>Phone *</label><input id="gp" name="phone" type="tel" autoComplete="tel" aria-invalid={!!errors.phone} aria-describedby={errors.phone ? 'gp-err' : undefined} className={field} placeholder="Best number" onChange={() => clearErr('phone')} />{errors.phone && <p id="gp-err" className="mt-1.5 text-xs text-[var(--color-blush-deep)]">{errors.phone}</p>}</div>
        <div className="sm:col-span-2"><label htmlFor="ge" className={label}>Email *</label><input id="ge" name="email" type="email" autoComplete="email" aria-invalid={!!errors.email} aria-describedby={errors.email ? 'ge-err' : undefined} className={field} placeholder="you@email.com" onChange={() => clearErr('email')} />{errors.email && <p id="ge-err" className="mt-1.5 text-xs text-[var(--color-blush-deep)]">{errors.email}</p>}</div>
