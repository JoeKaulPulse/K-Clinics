...
export function BookingFlow({ catalogue, client, preselect = null, preselectDate = '', waitlistToken = '' }: { catalogue: Service[]; client: ClientInfo; preselect?: string | null; preselectDate?: string; waitlistToken?: string }) {
  ...
  // BLD-1833: /book is now ISR-cached, so `client` above is always the
  // signed-out default at render time — the real signed-in state (name,
  // welcome-discount eligibility, SMS pref) is fetched here once mounted,
  // same split as the packages effect below.
  useEffect(() => {
    if (isDemo) return;
    let live = true;
